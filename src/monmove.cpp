...
float monster::rate_target( const Creature &c, float best, bool smart ) const
{
    ...
    const monster *mon = dynamic_cast< const monster * >( &c );
...
void monster::plan()
{
    ...
    if( friendly == 0 && ( turns_to_skip == 0 || turns_since_target % turns_to_skip == 0 ) ) {
        here.visit_reachable_creatures(*this, [this, &seen_levels, &mon_plan, &valid_targets](Creature& other) {
