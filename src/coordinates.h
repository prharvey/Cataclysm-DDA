 ...
 *
 * InBounds define if the point is guaranteed to be in bounds.
 *
...
class coord_point_mut : public coord_point_base<Point>
{
    ...
    public:
        ...
        template <typename T>
        explicit constexpr coord_point_mut( const Subpoint& p, T z ) : base(p.raw(), z) {}
        template<typename T>
    ...
    protected:
        // This constructor needs to be hidden, since it causes a large number of ambiguous conflicts
        // with the same tripoint constructor.
        template <typename T>
        explicit constexpr coord_point_mut( const point &p, T z ) : base( p, z ) {}
};
...
class coord_point : public coord_point_mut<Point, coord_point<point, Origin, Scale, InBounds>, InBounds>
{
    ...
    public:
        ...
        using this_as_point_ib = coord_point<point, Origin, Scale, true>;
        ...
        template <typename T>
        static constexpr coord_point make_unchecked(const this_as_point_ib& other, T z) {
            return coord_point( other.raw(), z);
        }
        template <typename T>
        ...
        // Allow implicit construction from in bounds versions.
        constexpr coord_point(const this_as_ib& other) : base(other.raw()) {}

        constexpr auto xy() const {
            return coord_point<point, Origin, Scale, InBounds>::make_unchecked( this->raw().xy() );
        }
...
struct project_to_impl<ScaleUp, 0, ResultScale, InBounds> {
    ...
    coord_point<Point, Origin, ResultScale, InBounds> operator()(
        ...
        // In bounds points are guaranteed to be inbounds after scaling up.
        //
        // e.g. point_bub_sm_ib scaled up to point_bub_ms_ib points to the top
        // left of the submap, which is still inbounds.
        return coord_point<Point, Origin, ResultScale, InBounds>::make_unchecked( multiply_xy( src.raw(), ScaleUp ) );
    }
};
...
struct project_to_impl<0, ScaleDown, ResultScale, true> {
    ...
    coord_point<Point, Origin, ResultScale, true> operator()(
        ...
        // In bounds points are guaranteed to be inbounds after scaling down.
        //
        ...
        return coord_point<Point, Origin, ResultScale, true>::make_unchecked(
            divide_xy_non_negative(src.raw(), ScaleDown));
    }
};
...
// *_sm_ms are always in bounds as remainder.
...
template<typename Point, origin RemainderOrigin, scale SourceScale, bool InBounds>
using remainder_inbounds_t = typename remainder_inbounds<Point, RemainderOrigin, SourceScale, InBounds>::type;
...
struct quotient_remainder_helper {
    ...
    using remainder_type_tripoint = remainder_inbounds_t<tripoint, RemainderOrigin, FineScale, InBounds>;
};
    ...
    const coord_point<point, Origin, SourceScale, InBounds> &src )
{
    ...
    remainder_inbounds_t<point, RemainderOrigin, SourceScale, InBounds> remainder = remainder_inbounds_t<point, RemainderOrigin, SourceScale, InBounds>::make_unchecked(src.raw() - quotient.raw() * ScaleDown );
    ...
    const coord_point<PointR, FineOrigin, FineScale, FineInBounds> &fine )
{
    ...
    using PointResult = decltype( PointL() + PointR() );
    const coord_point<PointL, CoarseOrigin, FineScale, FineInBounds> refined_coarse =
        project_to<FineScale>( coarse );
    return coord_point<PointResult, CoarseOrigin, FineScale, CoarseInBounds && FineInBounds>::make_unchecked( refined_coarse.raw() + fine.raw() );
}
...
using point_bub_sm_ib = coords::coord_point<point, coords::origin::reality_bubble, coords::sm, true>;
...
using tripoint_bub_sm_ib = coords::coord_point<tripoint, coords::origin::reality_bubble, coords::sm, true>;
          ...
          const coords::coord_point<Point, Origin, Scale, RhsInBounds> &loc2 )
{
    return coords::coord_point < Point, Origin, Scale, LhsInBounds &&
           RhsInBounds > ( ( loc1.raw() + loc2.raw() ) / 2 );
}
