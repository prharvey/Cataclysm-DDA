...
units::volume map_stack::max_volume() const
{
    if( myorigin->has_furn( location ) ) {
        return myorigin->furn( location )->max_volume;
    }
    return myorigin->ter( location )->max_volume;
}
...
void map::set_transparency_cache_dirty( const tripoint_bub_ms_ib &p, bool field )
{
    const tripoint_bub_sm_ib smp = project_to<coords::scale::submap>( p );
...
void map::set_seen_cache_dirty( const tripoint_bub_ms_ib&change_location )
{
...
bool map::memory_cache_dec_is_dirty( const tripoint_bub_ms_ib&p ) const
{
    return !get_cache( p.z() ).map_memory_cache_dec[p.x() + p.y() * MAPSIZE_Y];
}

void map::memory_cache_dec_set_dirty( const tripoint_bub_ms_ib&p, bool value ) const
{
    get_cache( p.z() ).map_memory_cache_dec[p.x() + p.y() * MAPSIZE_Y] = !value;
}

bool map::memory_cache_ter_is_dirty( const tripoint_bub_ms_ib&p ) const
{
    return !get_cache( p.z() ).map_memory_cache_ter[p.x() + p.y() * MAPSIZE_Y];
}

void map::memory_cache_ter_set_dirty( const tripoint_bub_ms_ib&p, bool value ) const
{
...
void map::memory_clear_vehicle_points( const vehicle &veh ) const
{
    ...
    for( const tripoint_bub_ms &p : veh.get_points() ) {
        if (const std::optional<tripoint_bub_ms_ib> ib = make_inbounds(p)) {
            memory_cache_dec_set_dirty(*ib, true);
            player_character.memorize_clear_decoration(abs_from_bub(*ib), "vp_");
        }
    }
}
...
void map::clear_vehicle_point_from_cache( vehicle *veh, const tripoint_bub_ms &pt )
{
    ...
    if( ch ) {
        if (const std::optional< tripoint_bub_ms_ib> ib = make_inbounds(pt)) {
            ch->set_veh_exists_at(ib->raw(), false);
        }
    ...
    for( int z = zmin; z <= zmax; z++ ) {
        ...
        for( int x = 0; x < getmapsize(); x++ ) {
            for( int y = 0; y < getmapsize(); y++ ) {
                tripoint_bub_sm_ib const grid = tripoint_bub_sm_ib::make_unchecked( x, y, z );
    ...
    for( size_t i = 0; i < current_submap->vehicles.size(); i++ ) {
        if( current_submap->vehicles[i].get() == veh ) {
            for( const tripoint_bub_ms &pt : veh->get_points() ) {
                if( const std::optional<tripoint_bub_ms_ib> p = make_inbounds(pt) ) {
                    memory_cache_dec_set_dirty( *p, true );
                }
                get_avatar().memorize_clear_decoration( abs_from_bub( pt ), "vp_" );
            }
            ch.vehicle_list.erase( veh );
            ch.zone_vehicles.erase( veh );
...
void map::vehmove()
{
    ...
    for( int zlev = minz; zlev <= maxz; ++zlev ) {
        ...
        for( vehicle *veh : cache.vehicle_list ) {
            if( veh->is_following ) {
                veh->drive_to_local_target( abs_from_bub( player_pos ), true );
...
vehicle *map::move_vehicle( vehicle &veh, const tripoint_rel_ms &dp, const tileray &facing )
{
    ...
    cata_assert( vertical == ( dp.xy() == point_rel_ms()) );
    ...
    if( impulse > 0 ) {
        ...
        sounds::sound( veh.pos_bub().raw(), volume, sounds::sound_t::combat, _("crash!"),
    ...
    if( !vertical && !veh.valid_wheel_config() && !( veh.is_watercraft() && veh.can_float() ) &&
        ...
        for( const tripoint_bub_ms &p : veh.get_points() ) {
            if (const std::optional<tripoint_bub_ms_ib> pb = make_inbounds(tripoint_bub_ms(p))) {
                const ter_id& pter = ter(*pb);
                if (pter == t_dirt || pter == t_grass) {
                    ter_set(*pb, t_dirtmound);
                }
            }
        }
    }
    ...
    if( !vertical && can_move ) {
        ...
        for( const int vp_wheel_idx : wheel_indices ) {
            ...
            if (const std::optional<tripoint_bub_ms_ib> wheel_p = make_inbounds(tripoint_bub_ms(veh.bub_part_pos(vp_wheel)))) {
...
bool map::deregister_vehicle_zone( zone_data &zone ) const
{
    ...
    if (const std::optional<tripoint_bub_ms_ib> p = make_inbounds(pos)) {
...
std::unordered_set<tripoint_bub_ms_ib> map::get_moving_vehicle_targets( const Creature &z, int max_range )
{
    const tripoint_bub_ms zpos = z.pos_bub();
    std::unordered_set<tripoint_bub_ms_ib> priority;
    std::unordered_set<tripoint_bub_ms_ib> visible;
    for( wrapped_vehicle &v : get_vehicles() ) {
        ...
        for( const vpart_reference &vpr : v.v->get_all_parts() ) {
            if (const std::optional<tripoint_bub_ms_ib> vppos = make_inbounds(vpr.pos_bub())) {
    ...
    for( int cx = chunk_sx; cx <= chunk_ex; ++cx ) {
        for( int cy = chunk_sy; cy <= chunk_ey; ++cy ) {
            for( int cz = chunk_sz; cz <= chunk_ez; ++cz ) {
                submap *current_submap = get_submap_at_grid( tripoint_bub_sm_ib::make_unchecked(cx, cy, cz) );
                if( current_submap == nullptr ) {
                ...
                for( const auto &elem : current_submap->vehicles ) {
                    ...
                    elem->sm_pos = tripoint_bub_sm_ib::make_unchecked(elem->sm_pos.xy(), cz);
                    wrapped_vehicle w;
                    w.v = elem.get();
                    w.pos = w.v->pos_bub();
                    vehs.push_back( w );
...
optional_vpart_position map::veh_at(const tripoint_bub_ms_ib& p) const
{
    if (!const_cast<map*>(this)->get_cache(p.z()).get_veh_in_active_range()) {
        return optional_vpart_position(std::nullopt);
    }
...
const vehicle *map::veh_at_internal( const tripoint_bub_ms_ib&p, int &part_num ) const
{
...
vehicle *map::veh_at_internal( const tripoint_bub_ms_ib&p, int &part_num )
{
    return const_cast<vehicle *>( const_cast<const map *>( this )->veh_at_internal( p, part_num ) );
}

void map::board_vehicle( const tripoint_bub_ms_ib&pos, Character *p )
{
    if( p == nullptr ) {
...
void map::unboard_vehicle( const tripoint_bub_ms_ib&p, bool dead_passenger )
{
...
{
    const tripoint_bub_ms src_ob = veh.pos_bub();
    // handle vehicle ramps
    int ramp_offset = 0;
    if( adjust_pos ) {
        if (const std::optional<tripoint_bub_ms_ib> p = make_inbounds(src_ob + dp)) {
            if (has_flag(ter_furn_flag::TFLAG_RAMP_UP, *p)) {
                ramp_offset += 1;
                veh.is_on_ramp = true;
            } else if (has_flag(ter_furn_flag::TFLAG_RAMP_DOWN, *p)) {
                ramp_offset -= 1;
                veh.is_on_ramp = true;
            }
        }
    ...
    const tripoint_bub_ms dst_ob = adjust_pos ? src_ob + dp + tripoint_rel_ms( 0, 0, ramp_offset ) : src_ob;

    const std::optional<tripoint_bub_ms_ib> src_opt = make_inbounds(src_ob);
    const std::optional<tripoint_bub_ms_ib> dst_opt = make_inbounds(dst_ob);

    if( !src_opt || !dst_opt ) {
        add_msg_debug( debugmode::DF_MAP,
                       "map::displace_vehicle: coordinates out of bounds %d,%d,%d->%d,%d,%d",
                       src_ob.x(), src_ob.y(), src_ob.z(), dst_ob.x(), dst_ob.y(), dst_ob.z() );
        return false;
    }
    const tripoint_bub_ms_ib& src = *src_opt;
    const tripoint_bub_ms_ib& dst = *dst_opt;

    submap *src_submap = get_submap_at(src);
    point_sm_ms_ib dst_offset;
    submap *const dst_submap = get_submap_at(dst, dst_offset);
    if( src_submap == nullptr || dst_submap == nullptr ) {
        debugmsg( "Tried to displace vehicle at (%d,%d) but the submap is not loaded", src.x(), src.y() );
        return true;
    }
    ...
    for( size_t i = 0; !complete && i < riders.size(); i++ ) {
        ...
        for( rider_data &r : riders ) {
            ...
            if (const std::optional<tripoint_bub_ms_ib> ib = make_inbounds(src + dp + next_pos)) {
                if (has_flag(ter_furn_flag::TFLAG_RAMP_UP, *ib)) {
                    psg_offset_z += 1;
                }
                else if (has_flag(ter_furn_flag::TFLAG_RAMP_DOWN, *ib)) {
                    psg_offset_z -= 1;
                }
            }
    ...
    const tripoint_bub_ms ob = dst;
    veh.shed_loose_parts( trinary::SOME, &ob);
...
bool map::displace_water( const tripoint_bub_ms_ib &p )
{
    ...
    if( has_flag_ter( ter_furn_flag::TFLAG_SHALLOW_WATER, p ) ) {
        ...
        for( int pass = 0; pass < 2; pass++ ) {
            ...
            for( const tripoint_bub_ms_ib&temp : points_in_radius( p, 1 ) ) {
                if( temp != p
...
void map::set( const tripoint_bub_ms_ib&p, const ter_id &new_terrain, const furn_id &new_furniture )
{
    furn_set( p, new_furniture );
    ter_set( p, new_terrain );
}

std::string map::name( const tripoint_bub_ms_ib&p )
{
    return has_furn( p ) ? furnname( p ) : tername( p );
}

std::string map::disp_name( const tripoint_bub_ms_ib&p )
{
    return string_format( _( "the %s" ), name( p ) );
}

std::string map::obstacle_name( const tripoint_bub_ms_ib&p )
{
...
bool map::has_furn( const tripoint_bub_ms_ib&p ) const
{
...
{
    point_sm_ms_ib l;
    const submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried process furniture at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return f_null;
    }
...
bool map::furn_set( const tripoint_bub_ms_ib&p, const furn_id &new_furniture, const bool furn_reset,
                    bool avoid_creatures )
{
    if( avoid_creatures ) {
        Creature *c = get_creature_tracker().creature_at( p.raw(), true);
        if( c ) {
            return false;
        }
    }
    ...
    if( current_submap == nullptr ) {
        debugmsg( "Tried to set furniture at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return false;
    }
    ...
    if( old_id == f_rubble && new_target_furniture == f_null ) {
        Creature *c = get_creature_tracker().creature_at( p.raw() );
        if( c ) {
    ...
    if( player_character.sees( p ) ) {
        player_character.memorize_clear_decoration( abs_from_bub( p ), "f_" );
    }
    ...
    support_dirty( p );
    if (const std::optional<tripoint_bub_ms_ib> above = make_inbounds(tripoint_bub_ms(p.xy(), p.z() + 1))) {
...
bool map::can_move_furniture( const tripoint_bub_ms_ib&pos, Character *you ) const
{
    if( !you ) {
        return false;
    }
...
std::string map::furnname( const tripoint_bub_ms_ib&p )
{
...
ter_id map::ter( const tripoint_bub_ms_ib&p ) const
{
    ...
    const submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to process terrain at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return t_null;
    }
...
int map::get_map_damage( const tripoint_bub_ms_ib&p ) const
{
    point_sm_ms_ib l;
    const submap *const current_submap = get_submap_at( p );
    if( current_submap == nullptr ) {
...
void map::set_map_damage( const tripoint_bub_ms_ib&p, int dmg )
{
    point_sm_ms_ib l;
    submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
...
uint8_t map::get_known_connections( const tripoint_bub_ms_ib&p,
                                    const std::bitset<NUM_TERCONN> &connect_group,
                                    const std::map<tripoint_bub_ms_ib, ter_id> &override ) const
{
    ...
    uint8_t val = 0;
    std::function<bool( const tripoint_bub_ms_ib& )> is_memorized;
    avatar &player_character = get_avatar();
#ifdef TILES
    if( use_tiles ) {
        is_memorized =
        [&]( const tripoint_bub_ms_ib& q ) {
            return !player_character.get_memorized_tile( abs_from_bub( q ) ).get_ter_id().empty();
        };
    } else {
#endif
        is_memorized =
        [&]( const tripoint_bub_ms_ib& q ) {
            return player_character.get_memorized_tile( abs_from_bub( q ) ).symbol != 0;
        };
#ifdef TILES
    }
#endif
    ...
    for( int i = 0; i < 4; ++i ) {
        const std::optional<tripoint_bub_ms_ib> neighbour_opt = make_inbounds(p + offsets[i]);
        if( !neighbour_opt) {
            continue;
        }
        const tripoint_bub_ms_ib& neighbour = *neighbour_opt;
...
uint8_t map::get_known_rotates_to( const tripoint_bub_ms_ib&p,
...
uint8_t map::get_known_connections_f( const tripoint_bub_ms_ib&p,
                                      const std::bitset<NUM_TERCONN> &connect_group,
                                      const std::map<tripoint_bub_ms_ib, furn_id> &override ) const
{
    if( connect_group.none() ) {
        return 0;
    }
...
#ifdef TILES
    if( use_tiles ) {
        is_memorized = [&]( const tripoint_bub_ms_ib& q ) {
            return !player_character.get_memorized_tile( abs_from_bub( q ) ).get_dec_id().empty();
        };
    } else {
#endif
        is_memorized = [&]( const tripoint_bub_ms_ib& q ) {
    ...
    // populate connection information
    for( int i = 0; i < 4; ++i ) {
        const std::optional<tripoint_bub_ms_ib> pt_opt = make_inbounds(p + offsets[i]);
        if (!pt_opt) {
            continue;
        }
        const tripoint_bub_ms_ib& pt = *pt_opt;
...
uint8_t map::get_known_rotates_to_f( const tripoint_bub_ms_ib&p,
                                     const std::bitset<NUM_TERCONN> &rotate_to_group,
                                     const std::map<tripoint_bub_ms_ib, ter_id> &override,
                                     const std::map<tripoint_bub_ms_ib, furn_id> &override_f ) const
{
    if( rotate_to_group.none() ) {
        return CHAR_MAX;
    }
 ...
 */
const harvest_id &map::get_harvest( const tripoint_bub_ms_ib&pos ) const
{
...
const std::set<std::string> &map::get_harvest_names( const tripoint_bub_ms_ib&pos ) const
{
 ...
 */
ter_id map::get_ter_transforms_into( const tripoint_bub_ms_ib&p ) const
{
    return ter( p )->transforms_into.id();
}
 ...
 */
void map::examine( Character &you, const tripoint_bub_ms_ib&pos ) const
{
    const furn_t furn_here = furn( pos ).obj();
    if( furn_here.can_examine( pos ) ) {
        furn_here.examine( dynamic_cast<Character &>( you ), pos );
    } else {
        ter( pos )->examine( dynamic_cast<Character &>( you ), pos );
    }
}

bool map::is_harvestable( const tripoint_bub_ms_ib&pos ) const
{
 ...
 */
bool map::ter_set( const tripoint_bub_ms_ib&p, const ter_id &new_terrain, bool avoid_creatures )
{
    if( avoid_creatures ) {
    ...
    submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to set terrain at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return true;
    }
    ...
    avatar &player_character = get_avatar();
    if( player_character.sees( p ) ) {
        player_character.memorize_clear_decoration( abs_from_bub( p ), "t_" );
    }
...
std::string map::tername( const tripoint_bub_ms_ib&p ) const
{
    return ter( p )->name();
}

std::string map::features( const tripoint_bub_ms_ib&p ) const
{
    std::string result;
...
bool map::is_wall_adjacent( const tripoint_bub_ms_ib&center ) const
{
    for( const tripoint_bub_ms_ib&p : points_in_radius( center, 1 ) ) {
...
bool map::is_open_air( const tripoint_bub_ms_ib&p ) const
{
    point_sm_ms_ib l;
    const submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
...
int map::move_cost( const tripoint_bub_ms_ib&p, const vehicle *ignored_vehicle ) const
{
    point_sm_ms_ib l;
    const submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
...
bool map::impassable( const tripoint_bub_ms_ib &p ) const
{
    return !passable( p );
}

bool map::passable( const tripoint_bub_ms_ib&p ) const
{
    return move_cost( p ) != 0;
}

int map::move_cost_ter_furn( const tripoint_bub_ms_ib&p ) const
{
    point_sm_ms_ib l;
    const submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried process terrain at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return 0;
    }

    const int tercost = current_submap->get_ter( l )->movecost;
    if( tercost == 0 ) {
        return 0;
    }

    const int furncost = current_submap->get_furn( l )->movecost;
    if( furncost < 0 ) {
        return 0;
    }
...
bool map::impassable_ter_furn( const tripoint_bub_ms_ib&p ) const
{
    return !passable_ter_furn( p );
}

bool map::passable_ter_furn( const tripoint_bub_ms_ib&p ) const
{
    return move_cost_ter_furn( p ) != 0;
}

int map::combined_movecost( const tripoint_bub_ms_ib&from, const tripoint_bub_ms_ib&to,
...
bool map::valid_move( const tripoint_bub_ms_ib&from, const tripoint_bub_ms_ib&to,
                      const bool bash, const bool flying, const bool via_ramp ) const
{
    ...
    const tripoint_bub_ms_ib&up_p = going_up ? to : from;
    const tripoint_bub_ms_ib&down_p = going_up ? from : to;
    ...
    if( has_floor( up_p ) && !up_ter.has_flag( ter_furn_flag::TFLAG_GOES_DOWN ) &&
        ...
        if( std::abs( from.x() - to.x()) == 1 || std::abs(from.y() - to.y()) == 1) {
            ...
            tripoint_bub_ms_ib midpoint = tripoint_bub_ms_ib::make_unchecked( down_p.xy(), up_p.z() );
...
double map::ranged_target_size( const tripoint_bub_ms_ib&p ) const
{
    if( impassable( p ) ) {
        return 1.0;
    }
...
int map::climb_difficulty( const tripoint_bub_ms_ib&p ) const
{
    ...
    for( const tripoint_bub_ms_ib&pt : points_in_radius( p, 1 ) ) {
...
bool map::has_floor( const tripoint_bub_ms_ib&p ) const
{
    if( !zlevels || p.z() < -OVERMAP_DEPTH + 1 ) {
        return true;
    }
...
bool map::has_floor_or_water( const tripoint_bub_ms_ib&p ) const
{
    if( !zlevels || p.z() < -OVERMAP_DEPTH + 1 ) {
        return true;
    }
...
bool map::supports_above( const tripoint_bub_ms_ib&p ) const
{
...
bool map::has_floor_or_support( const tripoint_bub_ms_ib&p ) const
{
    if (const std::optional<tripoint_bub_ms_ib> below = make_inbounds(p.xy(), p.z() - 1)) {
        return !valid_move(p, *below, false, true);
    }
    return true;
}

bool map::has_vehicle_floor( const tripoint_bub_ms_ib&p ) const
{
    return veh_at( p ).part_with_feature( "BOARDABLE", false ) ||
           veh_at( p ).part_with_feature( "OBSTACLE", false );
}

void map::drop_everything( const tripoint_bub_ms_ib&p )
{
...
void map::drop_furniture( const tripoint_bub_ms_ib&p )
{
    ...
    const auto check_tile = [this]( const tripoint_bub_ms_ib& pt ) {
        if( has_floor_or_water( pt ) ) {
            return SS_FLOOR;
        }

        if( supports_above( pt ) ) {
            return SS_GOOD_SUPPORT;
        }

        const furn_id frn_id = furn( pt );
        if( frn_id != f_null ) {
            const furn_t &frn = frn_id.obj();
        ...
        if( get_creature_tracker().creature_at( pt ) != nullptr ) {
            // Smash a critter
            return SS_CREATURE;
        }
    ...
    if( last_state == SS_FLOOR ) {
        ...
        if (const std::optional<tripoint_bub_ms_ib> below = make_inbounds(current.xy(), current.z() - 1)) {
...
void map::drop_items( const tripoint_bub_ms_ib&p )
{
    if( !has_items( p ) ) {
        return;
    }
    ...
    tripoint_bub_ms_ib below = p;
    int height_fallen = 0;
    while( !has_floor_or_water( below ) ) {
        below = tripoint_bub_ms_ib::make_unchecked(below.xy(), below.z() - 1);
        height_fallen++;
    }
...
void map::drop_vehicle( const tripoint_bub_ms_ib&p )
{
    const optional_vpart_position vp = veh_at( p );
    if( !vp ) {
        return;
...
void map::drop_fields( const tripoint_bub_ms_ib&p)
{
    ...
    const std::optional<tripoint_bub_ms_ib> below = make_inbounds(p + tripoint_below);
...
void map::support_dirty( const tripoint_bub_ms_ib&p )
{
    if( zlevels ) {
        support_cache_dirty.insert( p );
    }
...
void map::process_falling()
{
    ...
    if( !support_cache_dirty.empty() ) {
        ...
        std::unordered_set<tripoint_bub_ms_ib> last_cache = std::exchange( support_cache_dirty, std::unordered_set<tripoint_bub_ms_ib>());
        for( const tripoint_bub_ms_ib&p : last_cache ) {
            drop_everything( p );
        }
    }
}

bool map::has_flag( const std::string &flag, const tripoint_bub_ms_ib&p ) const
{
    return has_flag_ter_or_furn( flag, p ); // Does bound checking
}

bool map::can_put_items( const tripoint_bub_ms_ib&p ) const
{
    return can_put_items_ter_furn( p ) || veh_at( p ).cargo().has_value();
}

bool map::can_put_items_ter_furn( const tripoint_bub_ms_ib&p ) const
{
    return !has_flag( ter_furn_flag::TFLAG_NOITEM, p ) && !has_flag( ter_furn_flag::TFLAG_SEALED, p );
}

bool map::has_flag_ter( const std::string &flag, const tripoint_bub_ms_ib&p ) const
{
    return ter( p )->has_flag( flag );
}

bool map::has_flag_furn( const std::string &flag, const tripoint_bub_ms_ib&p ) const
{
    return furn( p )->has_flag( flag );
}

bool map::has_flag_ter_or_furn( const std::string &flag, const tripoint_bub_ms_ib&p ) const
{
    point_sm_ms_ib l;
    const submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
    ...
    return current_submap->get_ter( l )->has_flag( flag ) ||
           current_submap->get_furn( l )->has_flag( flag );
}

bool map::has_flag( const ter_furn_flag flag, const tripoint_bub_ms_ib&p ) const
{
    return has_flag_ter_or_furn( flag, p );
}

bool map::has_flag_ter( const ter_furn_flag flag, const tripoint_bub_ms_ib&p ) const
{
    return ter( p )->has_flag( flag );
}

bool map::has_flag_furn( const ter_furn_flag flag, const tripoint_bub_ms_ib&p ) const
{
    return furn( p )->has_flag( flag );
}

bool map::has_flag_ter_or_furn( const ter_furn_flag flag, const tripoint_bub_ms_ib&p ) const
{
    point_sm_ms_ib l;
    const submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
...
bool map::is_bashable( const tripoint_bub_ms_ib&p, const bool allow_floor ) const
{
    if( !inbounds( p ) ) {
    ...
    if( has_furn( p ) && furn( p )->bash.str_max != -1 ) {
        return true;
    }

    const map_bash_info &ter_bash = ter( p )->bash;
    return ter_bash.str_max != -1 && ( !ter_bash.bash_below || allow_floor );
}

bool map::is_bashable_ter( const tripoint_bub_ms_ib&p, const bool allow_floor ) const
{
    const map_bash_info &ter_bash = ter( p )->bash;
    return ter_bash.str_max != -1 && ( !ter_bash.bash_below || allow_floor );
}

bool map::is_bashable_furn( const tripoint_bub_ms_ib&p ) const
{
    return has_furn( p ) && furn( p )->bash.str_max != -1;
}

bool map::is_bashable_ter_furn( const tripoint_bub_ms_ib&p, const bool allow_floor ) const
{
    return is_bashable_furn( p ) || is_bashable_ter( p, allow_floor );
}

int map::bash_strength( const tripoint_bub_ms_ib&p, const bool allow_floor ) const
{
    if(is_bashable_furn(p)) {
        return furn( p )->bash.str_max;
    }
...
int map::bash_resistance( const tripoint_bub_ms_ib&p, const bool allow_floor ) const
{
    if(is_bashable_furn(p)) {
        return furn( p )->bash.str_min;
    }
...
int map::bash_rating( const int str, const tripoint_bub_ms_ib&p, const bool allow_floor ) const
{
    if( str <= 0 ) {
        return -1;
    }
...
void map::make_rubble( const tripoint_bub_ms_ib&p, const furn_id &rubble_type, const bool items,
...
bool map::is_water_shallow_current( const tripoint_bub_ms_ib&p ) const
{
    return has_flag( ter_furn_flag::TFLAG_CURRENT, p ) &&
           !has_flag( ter_furn_flag::TFLAG_DEEP_WATER, p );
}

bool map::is_divable( const tripoint_bub_ms_ib&p ) const
{
    return has_flag( ter_furn_flag::TFLAG_SWIMMABLE, p ) &&
           has_flag( ter_furn_flag::TFLAG_DEEP_WATER, p );
}

bool map::is_outside( const tripoint_bub_ms_ib&p ) const
{
    const auto &outside_cache = get_cache_ref( p.z() ).outside_cache;
    return outside_cache[p.x()][p.y()];
}

bool map::is_last_ter_wall( const bool no_furn, const point_bub_ms_ib&p,
                            const point_bub_ms_ib&max, const direction dir ) const
{
    point mov;
    switch( dir ) {
    ...
    while( loop && ( ( dir == direction::NORTH && p2.y() >= 0) ||
        ...
        point_bub_ms_ib ib = point_bub_ms_ib::make_unchecked(p2);
        if( no_furn && has_furn(ib) ) {
            loop = false;
            result = false;
        } else if( !has_flag_ter( ter_furn_flag::TFLAG_FLAT, ib) ) {
            loop = false;
            if( !has_flag_ter( ter_furn_flag::TFLAG_WALL, ib) ) {
                result = false;
            }
        }
        p2.x() += mov.x;
...
bool map::tinder_at( const tripoint_bub_ms_ib&p )
{
...
bool map::flammable_items_at( const tripoint_bub_ms_ib&p, int threshold )
{
    if( !has_items( p ) ||
...
bool map::is_flammable( const tripoint_bub_ms_ib&p )
{
...
void map::decay_fields_and_scent( const time_duration &amount )
{
    ...
    for( int smx = 0; smx < my_MAPSIZE; ++smx ) {
        for( int smy = 0; smy < my_MAPSIZE; ++smy ) {
            submap *cur_submap = get_submap_at_grid(tripoint_bub_sm_ib::make_unchecked( smx, smy, smz ) );
            ...
            for( int sx = 0; sx < SEEX; ++sx ) {
                ...
                for( int sy = 0; sy < SEEY; ++sy ) {
                    ...
                    const field &fields = cur_submap->get_field( point_sm_ms_ib::make_unchecked( sx, sy ));
                    ...
                    for( const auto &fp : fields ) {
                        ...
                        const int decay_amount_factor =  type->decay_amount_factor;
...
bool map::has_adjacent_furniture_with( const tripoint_bub_ms_ib&p,
                                       const std::function<bool( const furn_t & )> &filter ) const
{
    for( const tripoint_bub_ms_ib&adj : points_in_radius( p, 1 ) ) {
...
bool map::has_nearby_fire( const tripoint_bub_ms_ib&p, int radius ) const
{
    for( const tripoint_bub_ms_ib&pt : points_in_radius( p, radius ) ) {
...
bool map::has_nearby_table( const tripoint_bub_ms_ib&p, int radius ) const
{
    for( const tripoint_bub_ms_ib&pt : points_in_radius( p, radius ) ) {
...
bool map::has_nearby_chair( const tripoint_bub_ms_ib&p, int radius ) const
{
    for( const tripoint_bub_ms_ib&pt : points_in_radius( p, radius ) ) {
        const optional_vpart_position vp = veh_at( pt );
...
bool map::has_nearby_ter( const tripoint_bub_ms_ib&p, const ter_id &type, int radius ) const
{
    for( const tripoint_bub_ms_ib&pt : points_in_radius( p, radius ) ) {
        if( ter( pt ) == type ) {
            return true;
...
bool map::terrain_moppable( const tripoint_bub_ms_ib&p )
{
    ...
    for( const std::pair<const field_type_id, field_entry> &pr : field_at( p ) ) {
        if( pr.second.get_field_type()->phase == phase_id::LIQUID ) {
            return true;
        }
    }
...
bool map::mop_spills( const tripoint_bub_ms_ib&p )
{
    bool retval = false;
...
int map::collapse_check( const tripoint_bub_ms_ib&p ) const
{
    ...
    if( p.z() > -OVERMAP_DEPTH) {
        const tripoint_bub_ms_ib&pbelow = tripoint_bub_ms_ib::make_unchecked( p.xy(), p.z() - 1);
        for( const tripoint_bub_ms_ib&tbelow : points_in_radius( pbelow, 1 ) ) {
    ...
    for( const tripoint_bub_ms_ib&t : points_in_radius( p, 1 ) ) {
        if( p == t ) {
            continue;
        }
...
void map::collapse_at( const tripoint_bub_ms_ib&p, const bool silent, const bool was_supporting,
                       const bool destroy_pos )
{
    ...
    if( supports && !still_supports && p.z() < OVERMAP_HEIGHT ) {
        for( const tripoint_bub_ms_ib&t : points_in_radius( p, 1 ) ) {
            // If z-levels are off, tz == t, so we end up skipping a lot of stuff to avoid bugs.
            const tripoint_bub_ms_ib tz = tripoint_bub_ms_ib::make_unchecked( t.xy(), t.z() + 1);
...
void map::smash_items( const tripoint_bub_ms_ib&p, const int power, const std::string &cause_message )
{
...
ter_id map::get_roof( const tripoint_bub_ms_ib&p, const bool allow_air ) const
{
...
static bool furn_is_supported( const map &m, const tripoint_bub_ms_ib&p )
{
    static constexpr std::array<point_rel_ms, 4> cs = { { {0, -1},  {-1, 0}, {0, 1}, {1, 0} } };

    for( const point_rel_ms& c : cs) {
        if (const std::optional<tripoint_bub_ms_ib> adj = m.make_inbounds(p.xy() + c, p.z())) {
            if (m.has_furn(*adj) && m.furn(*adj)->has_flag(ter_furn_flag::TFLAG_BLOCKSDOOR)) {
                return true;
            }
        }
    }
...
void map::bash_ter_furn( const tripoint_bub_ms_ib&p, bash_params &params )
{
    int sound_volume = 0;
    ...
    if( smash_ter && bash->bash_below && ( !zlevels || !params.bash_floor ) ) {
        if( !params.destroy ) { // NOLINT(bugprone-branch-clone)
            smash_ter = false;
            bash = nullptr;
        } else if( !bash->ter_set && zlevels && p.z() > -OVERMAP_DEPTH ) {
            // HACK: A hack for destroy && !bash_floor
            // We have to check what would we create and cancel if it is what we have now
            tripoint_bub_ms_ib below = tripoint_bub_ms_ib::make_unchecked( p.xy(), p.z() - 1);
    ...
    if( !params.destroy ) {
        ...
        if(p.z() > -OVERMAP_DEPTH && (bash->str_min_supported != -1 || bash->str_max_supported != -1) ) {
            tripoint_bub_ms_ib below = tripoint_bub_ms_ib::make_unchecked( p.xy(), p.z() - 1);
    ...
    if( tent ) {
        ...
        std::optional<std::pair<tripoint_bub_ms_ib, furn_id>> tentp;
        ...
        if( centers.count( furn( p ) ) > 0 ) {
            ...
            for( const tripoint_bub_ms_ib&pt : points_in_radius( p, bash->collapse_radius ) ) {
        ...
        if( !tentp ) {
            ...
            // Take the tent down
            const int rad = tentp->second->bash.collapse_radius;
            for( const tripoint_bub_ms_ib&pt : points_in_radius( tentp->first, rad ) ) {
                const furn_id frn = furn( pt );
                if( frn == f_null ) {
                    continue;
                }

                const map_bash_info *recur_bash = &frn->bash;
        ...
        if( bash->bash_below && p.z() > -OVERMAP_DEPTH ) {
            tripoint_bub_ms_ib below = tripoint_bub_ms_ib::make_unchecked(p.xy(), p.z() - 1);
            const ter_t& ter_below = ter(below).obj();
            if (ter_below.has_flag(ter_furn_flag::TFLAG_SUPPORTS_ROOF)) {
    ...
    if( smash_ter && ter( p ) == t_open_air && zlevels && p.z() > -OVERMAP_DEPTH) {
        tripoint_bub_ms_ib below = tripoint_bub_ms_ib::make_unchecked(p.xy(), p.z() - 1);
    ...
    if( !sound.empty() && !params.silent ) {
        sounds::sound( p.raw(), sound_volume, sounds::sound_t::combat, sound, false,
                       soundfxid, soundfxvariant );
    }
}

bash_params map::bash( const tripoint_bub_ms_ib&p, const int str,
...
void map::bash_items( const tripoint_bub_ms_ib&p, bash_params &params )
{
    if( !has_items( p ) || has_flag_ter_or_furn( ter_furn_flag::TFLAG_PLANT, p ) ) {
        return;
    }
...
void map::bash_vehicle( const tripoint_bub_ms_ib&p, bash_params &params )
{
...
void map::bash_field( const tripoint_bub_ms_ib&p, bash_params &params )
{
...
void map::destroy( const tripoint_bub_ms_ib&p, const bool silent )
{
...
void map::destroy_furn( const tripoint_bub_ms_ib&p, const bool silent )
{
    // Break if it takes more than 25 destructions to remove to prevent infinite loops
    // Example: A bashes to B, B bashes to A leads to A->B->A->...
    int count = 0;
...
void map::batter( const tripoint_bub_ms_ib&p, int power, int tries, const bool silent )
{
    int count = 0;
...
void map::crush( const tripoint_bub_ms_ib&p )
{
    creature_tracker &creatures = get_creature_tracker();
...
void map::shoot( const tripoint_bub_ms_ib&p, projectile &proj, const bool hit_items )
{
...
bool map::hit_with_acid( const tripoint_bub_ms_ib&p )
{
    if( passable( p ) ) {
...
bool map::hit_with_fire( const tripoint_bub_ms_ib&p )
{
    if( passable( p ) ) {
        return false;    // Didn't hit the tile!
    }
...
bool map::open_door( Creature const &u, const tripoint_bub_ms_ib&p, const bool inside,
...
void map::translate( const ter_id &from, const ter_id &to )
{
    ...
    for( const tripoint_bub_ms_ib&p : points_on_zlevel() ) {
        if( ter( p ) == from ) {
            ter_set( p, to );
        }
    }
...
{
    if( from == to ) {
        debugmsg( "map::translate %s => %s", from->name(), to->name() );
        return;
    }

    const tripoint_abs_omt abs_omt_p = project_to<coords::scale::overmap_terrain>( abs_from_bub( p ) );
    for( const tripoint_bub_ms_ib&t : points_on_zlevel() ) {
        const tripoint_abs_omt abs_omt_t = project_to<coords::scale::overmap_terrain>( abs_from_bub( t ) );
...
bool map::close_door( const tripoint_bub_ms_ib&p, const bool inside, const bool check_only )
{
    if( has_flag( ter_furn_flag::TFLAG_OPENCLOSE_INSIDE, p ) && !inside ) {
        return false;
    }
...
std::string map::get_signage( const tripoint_bub_ms_ib&p ) const
{
    point_sm_ms_ib l;
    const submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to get signage at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return std::string();
    }

    return current_submap->get_signage( l );
}
void map::set_signage( const tripoint_bub_ms_ib&p, const std::string &message )
{
    point_sm_ms_ib l;
    submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried set signage at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return;
    }

    current_submap->set_signage( l, message );
}
void map::delete_signage( const tripoint_bub_ms_ib&p )
{
    point_sm_ms_ib l;
    submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to delete signage at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return;
    }
...
int map::get_radiation( const tripoint_bub_ms_ib&p ) const
{
    point_sm_ms_ib l;
    const submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to get radiation at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return 0;
    }
...
void map::set_radiation( const tripoint_bub_ms_ib&p, const int value )
{
    point_sm_ms_ib l;
    submap *const current_submap =get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to set radiation at (%d,%d,%d) but the submap is not loaded", p.x(), p.y());
        return;
    }
...
void map::adjust_radiation( const tripoint_bub_ms_ib&p, const int delta )
{
    point_sm_ms_ib l;
    submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to adjust radiation at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return;
    }
...
units::temperature_delta map::get_temperature_mod( const tripoint_bub_ms_ib&p ) const
{
    const submap *const current_submap = get_submap_at( p );
    if( current_submap == nullptr ) {
...
void map::set_temperature_mod( const tripoint_bub_ms_ib&p, units::temperature_delta new_temperature_mod )
{
...
map_stack map::i_at( const tripoint_bub_ms_ib&p )
{
    point_sm_ms_ib l;
    submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to get items at (%d,%d) but the submap is not loaded", p.x(), p.y());
        nulitems.clear();
...
map_stack::iterator map::i_rem( const tripoint_bub_ms_ib&p, const map_stack::const_iterator &it )
{
    point_sm_ms_ib l;
    submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to remove items at (%d,%d) but the submap is not loaded", p.x(), p.y());
        nulitems.clear();
...
void map::i_rem( const tripoint_bub_ms_ib&p, item *it )
{
...
void map::i_clear( const tripoint_bub_ms_ib&p )
{
    point_sm_ms_ib l;
    submap *const current_submap = get_submap_at( p,  l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to clear items at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return;
    }
...
std::vector<item *> map::spawn_items( const tripoint_bub_ms_ib&p, const std::vector<item> &new_items )
{
    std::vector<item *> ret;
...
void map::spawn_artifact( const tripoint_bub_ms_ib&p, const relic_procgen_id &id,
...
void map::spawn_item( const tripoint_bub_ms_ib&p, const itype_id &type_id, const unsigned quantity,
...
units::volume map::max_volume( const tripoint_bub_ms_ib&p )
{
    return i_at( p ).max_volume();
}

// total volume of all the things
units::volume map::stored_volume( const tripoint_bub_ms_ib&p )
{
    return i_at( p ).stored_volume();
}

// free space
units::volume map::free_volume( const tripoint_bub_ms_ib&p )
{
    return i_at( p ).free_volume();
}

item_location map::add_item_ret_loc( const tripoint_bub_ms_ib&pos, item obj, bool overflow )
{
    int copies = 1;
    std::pair<item *, tripoint_bub_ms_ib> ret = _add_item_or_charges( pos, std::move( obj ), copies, overflow );
    if( ret.first != nullptr && !ret.first->is_null() ) {
        return item_location { map_cursor{ abs_from_bub(ret.second) }, ret.first };
    }

    return {};
}

item &map::add_item_or_charges( const tripoint_bub_ms_ib&pos, item obj, bool overflow )
{
    int copies = 1;
    return *_add_item_or_charges( pos, std::move( obj ), copies, overflow ).first;
}

item &map::add_item_or_charges( const tripoint_bub_ms_ib&pos, item obj, int &copies_remaining,
                                bool overflow )
{
    return *_add_item_or_charges( pos, std::move( obj ), copies_remaining, overflow ).first;
}

std::pair<item *, tripoint_bub_ms_ib> map::_add_item_or_charges( const tripoint_bub_ms_ib&pos, item obj,
        int &copies_remaining, bool overflow )
{
    // Checks if item would not be destroyed if added to this tile
    auto valid_tile = [&]( const tripoint_bub_ms_ib& e ) {
    ...
    auto how_many_copies_fit = [&]( const tripoint_bub_ms_ib& e ) {
    ...
    auto place_item = [&]( const tripoint_bub_ms_ib& tile, int &copies ) -> item& {
    ...
    if( item_is_blacklisted( obj.typeId() ) ) {
        return { &null_item_reference(), tripoint_bub_ms_ib() };
    }
    ...
    std::optional<std::pair<item *, tripoint_bub_ms_ib>> first_added;
    ...
    if( overflow && copies_remaining > 0 ) {
        ...
        const pathfinding_settings setting( 0, max_dist, max_path_length, 0, false, false, true, false,
                                            false, false );
        for( const tripoint_bub_ms& e_ob : tiles ) {
            ...
            const tripoint_bub_ms_ib e = tripoint_bub_ms_ib::make_unchecked(e);
            //must be a path to the target tile
            if( route( pos, e, setting ).empty() ) {
                continue;
            }
            if( obj.made_of( phase_id::LIQUID ) || !obj.has_flag( flag_DROP_ACTION_ONLY_IF_LIQUID ) ) {
                if( obj.on_drop( e.raw(), *this)) {
                    return first_added ? first_added.value() : std::make_pair( &null_item_reference(), tripoint_bub_ms_ib());
                }
            }
            ...
            copies_remaining -= copies_to_add_here;
            std::pair<item *, tripoint_bub_ms_ib> new_item = { &place_item( e, copies_to_add_here ), e };
...
item &map::add_item( const tripoint_bub_ms_ib&p, item new_item, int copies )
{
    ...
    submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to add items at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return null_item_reference();
    }
    ...
    if( new_item.has_temperature() && !new_item.is_corpse() ) {
        new_item.process( *this, nullptr, p.raw() );
    }
    ...
    if( new_item.is_map() && !new_item.has_var( "reveal_map_center_omt" ) ) {
        new_item.set_var( "reveal_map_center_omt", project_to<coords::scale::overmap_terrain>( abs_from_bub( p ) ).raw() );
    }

    if( new_item.has_flag( json_flag_PRESERVE_SPAWN_OMT ) &&
        !new_item.has_var( "spawn_location_omt" ) ) {
        new_item.set_var( "spawn_location_omt", project_to<coords::scale::overmap_terrain>(abs_from_bub(p)).raw() );
    }
    for( item *const it : new_item.all_items_top( pocket_type::CONTAINER ) ) {
        if( it->has_flag( json_flag_PRESERVE_SPAWN_OMT ) &&
            ...
            it->set_var( "spawn_location_omt", project_to<coords::scale::overmap_terrain>(abs_from_bub(p)).raw());
        }
    }
    ...
    if( current_submap->active_items.add( *new_pos, l.raw() ) ) {
...
// NOLINTNEXTLINE(readability-make-member-function-const)
item map::water_from( const tripoint_bub_ms_ib&p )
{
    ...
    if( terrain_id->has_examine( iexamine::water_source ) ) {
        int poison_chance = 0;
        if( terrain_id->has_flag( ter_furn_flag::TFLAG_DEEP_WATER ) ) {
            if( terrain_id->has_flag( ter_furn_flag::TFLAG_CURRENT ) ) {
                poison_chance = 20;
            } else {
                poison_chance = 4;
            }
...
void map::make_active( item_location &loc )
{
    ...
    const tripoint_bub_ms& p = loc.pos_bub();
    const std::optional<tripoint_bub_ms_ib> pos = make_inbounds(p);
    point_sm_ms_ib l;
    submap *const current_submap = pos ? get_submap_at(*pos, l) : nullptr;
    if( current_submap == nullptr ) {
        debugmsg( "Tried to make active at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return;
    }
    ...
    if( current_submap->active_items.add( *iter, l.raw() ) ) {
        // TODO: fix point types
        tripoint_abs_sm const smloc( abs_sub.x() + p.x() / SEEX,
...
void map::update_lum( item_location &loc, bool add )
{
    ...
    const tripoint_bub_ms_ib p = loc.pos_bub();
    point_sm_ms_ib l;
    submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to update lum at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return;
    }
...
static bool process_map_items( map &here, item_stack &items, safe_reference<item> &item_ref,
                               item *parent, const tripoint_bub_ms&location, const float insulation,
                               const temperature_flag flag, const float spoil_multiplier )
{
    if( item_ref->process( here, nullptr, location.raw(), insulation, flag, spoil_multiplier, false)) {
        // Item is to be destroyed so erase it from the map stack
        // unless it was already destroyed by processing.
        if( item_ref ) {
            item_ref->spill_contents( location.raw() );
            if( parent != nullptr ) {
...
void map::process_items()
{
    ...
    for( int gz = minz; gz <= maxz; ++gz ) {
        ...
        std::unordered_set<tripoint_bub_sm_ib> submaps_with_vehicles;
        for( vehicle *this_vehicle : cache.vehicle_list ) {
            if (const std::optional<tripoint_bub_ms_ib> pos = make_inbounds(tripoint_bub_ms(this_vehicle->pos_bub()))) {
                submaps_with_vehicles.emplace(project_to<coords::scale::submap>(*pos));
            }
        }
        for( const tripoint_bub_sm_ib&pos : submaps_with_vehicles ) {
    ...
    for( auto iter = submaps_with_active_items.begin(); iter != submaps_with_active_items.end(); ) {
        if (const std::optional<tripoint_bub_sm_ib> ib = make_inbounds(bub_from_abs(*iter))) {
            submap* const current_submap = get_submap_at_grid(*ib);
            if (current_submap == nullptr) {
                debugmsg("Tried to process items at %s but the submap is not loaded", ib->to_string());
                continue;
            }
            process_items_in_submap(*current_submap, *ib);
...
void map::process_items_in_submap( submap &current_submap, const tripoint_bub_sm_ib&gridp )
{
    ...
    for( item_reference &active_item_ref : active_items ) {
        ...
        const tripoint_bub_ms_ib map_location = project_combine(gridp, active_item_ref.location);
...
void map::process_items_in_vehicle( vehicle &cur_veh, submap &current_submap )
{
    ...
    for( item_reference &active_item_ref : cur_veh.active_items.get_for_processing() ) {
        ...
        const auto it = std::find_if( begin( cargo_parts ),
            ...
            return point_rel_ms(active_item_ref.location.raw()) == part.mount();
        } );
...
bool map::sees_some_items( const tripoint_bub_ms_ib &p, const Creature &who ) const
{
    if (const std::optional<tripoint_bub_ms_ib> ib = make_inbounds(who.pos_bub())) {
        return sees_some_items(p, *ib);
    }
    return false;
}

bool map::sees_some_items( const tripoint_bub_ms_ib&p, const tripoint_bub_ms_ib&from ) const
{
    // Can only see items if there are any items.
    return has_items( p ) && could_see_items( p, from );
}

bool map::could_see_items( const tripoint_bub_ms_ib&p, const tripoint_bub_ms_ib&from ) const
{
...
bool map::has_items( const tripoint_bub_ms_ib&p ) const
{
    point_sm_ms_ib l;
    const submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to check items at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return false;
    }
    return !current_submap->get_items( l ).empty();
}

bool map::only_liquid_in_liquidcont( const tripoint_bub_ms_ib&p )
{
    if( has_flag( ter_furn_flag::TFLAG_LIQUIDCONT, p ) ) {
...
std::list<item> map::use_amount_square( const tripoint_bub_ms_ib&p, const itype_id &type,
...
std::list<item_location> map::items_with( const tripoint_bub_ms_ib&p,
        const std::function<bool( const item & )> &filter )
{
    ...
    for( item &it : i_at( p ) ) {
        if( filter( it ) ) {
            ret.emplace_back( map_cursor( abs_from_bub( p ) ), &it );
        }
    }
    return ret;
}
std::list<item> map::use_amount( const std::vector<tripoint_bub_ms_ib> &reachable_pts, const itype_id &type,
                                 int &quantity, const std::function<bool( const item & )> &filter, bool select_ind )
{
    ...
    if( select_ind && !type->count_by_charges() ) {
        ...
        for( const tripoint_bub_ms_ib&p : reachable_pts ) {
...
std::list<item> map::use_amount( const tripoint_bub_ms_ib&origin, const int range, const itype_id &type,
                                 int &quantity, const std::function<bool( const item & )> &filter, bool select_ind )
{
    std::vector<tripoint_bub_ms_ib> reachable_pts;
...
static void use_charges_from_furn( const furn_t &f, const itype_id &type, int &quantity,
                                   map *m, const tripoint_bub_ms_ib&p, std::list<item> &ret,
...
std::list<item> map::use_charges( const std::vector<tripoint_bub_ms_ib> &reachable_pts,
...
std::list<item> map::use_charges( const tripoint_bub_ms_ib&origin, const int range,
                                  const itype_id &type, int &quantity,
                                  const std::function<bool( const item & )> &filter,
                                  basecamp *bcp, bool in_tools )
{
...
units::energy map::consume_ups( const std::vector<tripoint_bub_ms_ib> &reachable_pts, units::energy qty )
{
...
units::energy map::consume_ups( const tripoint_bub_ms_ib&origin, const int range, units::energy qty )
{
    // populate a grid of spots that can be reached
    std::vector<tripoint_bub_ms_ib> reachable_pts;
    reachable_flood_steps( reachable_pts, origin, range, 1, 100 );
...
std::list<std::pair<tripoint_bub_ms_ib, item *> > map::get_rc_items( const tripoint_bub_ms_ib&p )
{
    std::list<std::pair<tripoint_bub_ms_ib, item *> > rc_pairs;
    tripoint_bub_ms pos;
    pos.z() = abs_sub.z();
    for( pos.x() = 0; pos.x() < MAPSIZE_X; pos.x()++) {
        ...
        for( pos.y() = 0; pos.y() < MAPSIZE_Y; pos.y()++) {
            ...
            tripoint_bub_ms_ib p2 = tripoint_bub_ms_ib::make_unchecked(pos);
            map_stack items = i_at(p2);
            for( item &elem : items ) {
                if( elem.has_flag( flag_RADIO_ACTIVATION ) || elem.has_flag( flag_RADIO_CONTAINER ) ) {
                    rc_pairs.emplace_back(p2, &elem );
                }
            }
        }
    }
...
bool map::can_see_trap_at( const tripoint_bub_ms_ib&p, const Character &c ) const
{
    return tr_at( p ).can_see( p.raw(), c);
}

const trap &map::tr_at( const tripoint_bub_ms_ib&p ) const
{
    point_sm_ms_ib l;
    const submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to get trap at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return tr_null.obj();
    }
...
partial_con *map::partial_con_at( const tripoint_bub_ms_ib&p )
{
    point_sm_ms_ib l;
    submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to get construction at %s but the submap is not loaded", p.to_string() );
        return nullptr;
    }
    auto it = current_submap->partial_constructions.find(tripoint_sm_ms_ib::make_unchecked(l, p.z()));
...
void map::partial_con_remove( const tripoint_bub_ms_ib&p )
{
    point_sm_ms_ib l;
    submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
    ...
    current_submap->partial_constructions.erase(tripoint_sm_ms_ib::make_unchecked(l, p.z()));
    memory_cache_dec_set_dirty( p, true );
    avatar &player_character = get_avatar();
    if( player_character.sees( p ) ) {
        player_character.memorize_clear_decoration( abs_from_bub( p ), "tr_" );
    }
}

void map::partial_con_set( const tripoint_bub_ms_ib&p, const partial_con &con )
{
    point_sm_ms_ib l;
    submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to set construction at %s but the submap is not loaded", p.to_string() );
        return;
    }
    if( !current_submap->partial_constructions.emplace(tripoint_sm_ms_ib::make_unchecked(l, p.z()), con).second) {
        debugmsg( "set partial con on top of terrain which already has a partial con" );
    }
}

void map::trap_set( const tripoint_bub_ms_ib&p, const trap_id &type )
{
    point_sm_ms_ib l;
    submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to set trap at %s but the submap is not loaded", p.to_string() );
        return;
    }
...
void map::remove_trap( const tripoint_bub_ms_ib&p )
{
    point_sm_ms_ib l;
    submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to remove trap at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return;
    }
...
const field &map::field_at( const tripoint_bub_ms_ib&p ) const
{
    ...
    const submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to get field at (%d,%d) but the submap is not loaded", p.x(), p.y(), p.z());
        nulfield = field();
        return nulfield;
    }
 ...
 */
field &map::field_at( const tripoint_bub_ms_ib&p )
{
    point_sm_ms_ib l;
    submap* const current_submap = get_submap_at(p, l);
    if (current_submap == nullptr) {
...
time_duration map::mod_field_age( const tripoint_bub_ms_ib&p, const field_type_id &type,
                                  const time_duration &offset )
{
    return set_field_age( p, type, offset, true );
}

int map::mod_field_intensity( const tripoint_bub_ms_ib&p, const field_type_id &type, const int offset )
{
    return set_field_intensity( p, type, offset, true );
}

time_duration map::set_field_age( const tripoint_bub_ms_ib&p, const field_type_id &type,
 ...
 */
int map::set_field_intensity( const tripoint_bub_ms_ib&p, const field_type_id &type,
...
time_duration map::get_field_age( const tripoint_bub_ms_ib&p, const field_type_id &type ) const
{
    const field_entry *field_ptr = field_at( p ).find_field( type );
    return field_ptr == nullptr ? -1_turns : field_ptr->get_field_age();
}

int map::get_field_intensity( const tripoint_bub_ms_ib&p, const field_type_id &type ) const
{
    const field_entry *field_ptr = get_field( p, type );
    return field_ptr == nullptr ? 0 : field_ptr->get_field_intensity();
}

bool map::has_field_at( const tripoint_bub_ms_ib&p) const
{
    const point_bub_sm_ib sm = project_to<coords::scale::submap>( p.xy() );
    return get_cache( p.z() ).field_cache[sm.x() + sm.y() * MAPSIZE];
}

bool map::has_field_at( const tripoint_bub_ms_ib&p, const field_type_id &type ) const
{
...
cata::copy_const<Map, field_entry> *map::get_field_helper(
    Map &m, const tripoint_bub_ms_ib&p, const field_type_id &type )
{
    if( !m.has_field_at( p, false ) ) {
        return nullptr;
    }

    point_sm_ms_ib l;
    auto *const current_submap = m.get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to get field at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return nullptr;
    }
...
field_entry *map::get_field( const tripoint_bub_ms_ib&p, const field_type_id &type )
{
    return get_field_helper( *this, p, type );
}

const field_entry *map::get_field( const tripoint_bub_ms_ib&p, const field_type_id &type ) const
{
    return get_field_helper( *this, p, type );
}

bool map::dangerous_field_at( const tripoint_bub_ms_ib&p )
{
...
bool map::mopsafe_field_at( const tripoint_bub_ms_ib&p )
{
...
bool map::add_field( const tripoint_bub_ms_ib&p, const field_type_id &type_id, int intensity,
                     const time_duration &age, bool hit_player )
{
    ...
    if( has_flag( ter_furn_flag::TFLAG_NO_FLOOR, p ) && type_id->phase != phase_id::GAS ) {
        return false;
    }

    // Don't spawn liquid fields on water tiles
    if( has_flag( ter_furn_flag::TFLAG_SWIMMABLE, p ) && type_id->phase == phase_id::LIQUID ) {
        return false;
    }
    ...
    submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to add field at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return false;
    }
    current_submap->ensure_nonuniform();
    invalidate_max_populated_zlev( p.z());
    ...
    if( current_submap->get_field( l ).add_field( converted_type_id, intensity, age ) ) {
        ...
        if( !current_submap->field_count++ ) {
            const point_bub_sm_ib s = project_to<coords::scale::submap>(p.xy());
            get_cache( p.z() ).field_cache.set(s.x() + s.y() * MAPSIZE);
        }
    }
...
void map::remove_field( const tripoint_bub_ms_ib&p, const field_type_id &field_to_remove )
{
    set_field_intensity( p, field_to_remove, 0 );
}

void map::clear_fields( const tripoint_bub_ms_ib&p )
{
    point_sm_ms_ib l;
    get_submap_at(p, l)->clear_fields( l );
}

void map::on_field_modified( const tripoint_bub_ms_ib&p, const field_type &fd_type )
{
    invalidate_max_populated_zlev( p.z() );
...
void map::add_splatter( const field_type_id &type, const tripoint_bub_ms_ib&where, int intensity )
{
    if( intensity <= 0 ) {
        return;
    }
    if( type->is_splattering ) {
...
void map::add_splatter_trail( const field_type_id &type, const tripoint_bub_ms_ib&from,
                              const tripoint_bub_ms_ib&to )
{
    if( !type.id() ) {
        return;
    }
    ...
    for( const tripoint_bub_ms_ib&elem : trail ) {
                      ...
                      int intensity )
{
    ...
    for( const tripoint_bub_ms_ib&pnt : points_in_radius( center, radius ) ) {
...
computer *map::computer_at( const tripoint_bub_ms_ib&p )
{
    point_sm_ms_ib l;
    submap *const sm = get_submap_at( p, l );
    if( sm == nullptr ) {
        debugmsg( "Tried to get computer at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return nullptr;
    }
...
{
    // TODO: fix point types
    const tripoint_abs_omt omt_check(project_to<coords::scale::overmap_terrain>( point_check ) );
...
void map::remove_submap_camp( const tripoint_bub_ms_ib&p )
{
    submap *const current_submap = get_submap_at( p );
    if( current_submap == nullptr ) {
...
basecamp map::hoist_submap_camp( const tripoint_bub_ms_ib&p )
{
    submap *const current_submap = get_submap_at( p );
    if( current_submap == nullptr ) {
...
void map::update_visibility_cache( const int zlev )
{
    ...
    for( int x = 0; x < MAPSIZE_X; x++ ) {
        for( int y = 0; y < MAPSIZE_Y; y++ ) {
            const tripoint_bub_ms_ib p = tripoint_bub_ms_ib::make_unchecked(x, y, zlev);
            lit_level ll = apparent_light_at( p, visibility_variables_cache );
            visibility_cache[p.xy()] = ll;
            sm_squares_seen[ project_to<coords::scale::submap>(p.xy()) ] += (ll == lit_level::BRIGHT || ll == lit_level::LIT);
        }
    }
    ...
    for( int gridx = 0; gridx < my_MAPSIZE; gridx++ ) {
        for( int gridy = 0; gridy < my_MAPSIZE; gridy++ ) {
            if( sm_squares_seen[gridx][gridy] > 36 ) { // 25% of the submap is visible
                const tripoint_rel_sm sm( gridx, gridy, 0 );
...
{
    const memorized_tile &mt = get_avatar().get_memorized_tile( get_map().abs_from_bub( p ) );
    if( mt.symbol != 0 ) {
        return mt.symbol;
    }
    return std::nullopt;
...
void map::draw( const catacurses::window &w, const tripoint_bub_ms_ib&center )
{
    ...
    int wnd_h = getmaxy( w );
    int wnd_w = getmaxx( w );
    const tripoint_bub_ms offs = center - tripoint( wnd_w / 2, wnd_h / 2, 0 );
    ...
    player_character.prepare_map_memory_region(
        abs_from_bub( tripoint_bub_ms( min_mm_reg, center.z() ) ),
        abs_from_bub( tripoint_bub_ms( max_mm_reg, center.z() ) )
    );
    ...
    for( int wy = 0; wy < wnd_h; wy++ ) {
        for( int wx = 0; wx < wnd_w; wx++ ) {
            wmove( w, point( wx, wy ) );
            const std::optional<tripoint_bub_ms_ib> p = make_inbounds(offs + point_rel_ms(wx, wy));
            if( !p ) {
                draw_background( *p );
                continue;
            }

            const lit_level lighting = visibility_cache[p->xy()];
            ...
            const const_maptile curr_maptile = maptile_at( *p );
            params.low_light( lighting == lit_level::LOW ).bright_light( lighting == lit_level::BRIGHT );
            if( draw_maptile( w, *p, curr_maptile, params ) ) {
                continue;
            }
            if (const std::optional<tripoint_bub_ms_ib> below = make_inbounds(*p + tripoint_below)) {
    ...
    for( int y = 0; y < MAPSIZE_Y; y++ ) {
        for( int x = 0; x < MAPSIZE_X; x++ ) {
            const tripoint_bub_ms_ib p = tripoint_bub_ms_ib::make_unchecked( x, y, center.z() );
            ...
            mm_params.low_light( lighting == lit_level::LOW ).bright_light( lighting == lit_level::BRIGHT );
...
void map::drawsq( const catacurses::window &w, const tripoint_bub_ms_ib&p,
...
bool map::dont_draw_lower_floor( const tripoint_bub_ms_ib&p ) const
{
    return !zlevels || p.z() <= -OVERMAP_DEPTH ||
           ( has_floor( p ) && !has_flag( ter_furn_flag::TFLAG_Z_TRANSPARENT, p ) );
}

bool map::draw_maptile( const catacurses::window &w, const tripoint_bub_ms_ib&p,
                        const const_maptile &curr_maptile, const drawsq_params &params ) const
{
    ...
    if( curr_trap.can_see( p.raw(), player_character)) {
    ...
    if( curr_field.field_count() > 0 ) {
        ...
        if( field_symbol == "&" || fe == nullptr ) {
            ...
            if( ( field_symbol != "%" && fid->priority > 1 ) || ( field_symbol != "%" &&
                    sym == '.' ) )  {
                // default terrain '.' and
    ...
    if( veh != nullptr ) {
        ...
        if( !veh->forward_velocity() && !veh->player_in_control( player_character )
            && !( player_character.get_grab_type() == object_type::VEHICLE
                  && veh->get_points().count( player_character.pos_bub() + player_character.grab_point ) ) ) {
            memory_sym = sym;
        }
    }

    if( param.memorize() && memory_cache_ter_is_dirty( p ) ) {
        player_character.memorize_symbol( abs_from_bub( p ), memory_sym );
...
void map::draw_from_above( const catacurses::window &w, const tripoint_bub_ms_ib&p,
...
bool map::sees( const tripoint_bub_ms_ib&F, const tripoint_bub_ms_ib&T, const int range ) const
{
    int dummy = 0;
 ...
 **/
bool map::sees( const tripoint_bub_ms_ib&F, const tripoint_bub_ms_ib&T, const int range,
                int &bresenham_slope ) const
{
    ...
    const tripoint_bub_ms_ib&min = F < T ? F : T;
    const tripoint_bub_ms_ib&max = !( F < T ) ? F : T;
    ...
    if( !fov_3d || F.z() == T.z()) {
        ...
        [this, &visible, &T]( const point & new_point ) {
            ...
            if( !this->is_transparent(tripoint_bub_ms_ib::make_unchecked( new_point.x, new_point.y, T.z() ) ) ) {
                visible = false;
                return false;
            }
            return true;
    ...
    [this, &visible, &end, &last_point]( const tripoint & new_point ) {
        // Exit before checking the last square if it's not a vertical transition,
        // it's still visible even if opaque.
        if( new_point == end && last_point.z == end.z ) {
            return false;
        }

        // TODO: Allow transparent floors (and cache them!)
        if( new_point.z == last_point.z ) {
            if( !this->is_transparent(tripoint_bub_ms_ib::make_unchecked(new_point) ) ) {
                visible = false;
                return false;
            }
        } else {
            const int max_z = std::max( new_point.z, last_point.z );
            if( ( has_floor_or_support(tripoint_bub_ms_ib::make_unchecked(new_point.xy(), max_z) ) ||
                  !is_transparent(tripoint_bub_ms_ib::make_unchecked(new_point.xy(), last_point.z) ) ) &&
                ( has_floor_or_support(tripoint_bub_ms_ib::make_unchecked(last_point.xy(), max_z) ) ||
                  !is_transparent(tripoint_bub_ms_ib::make_unchecked(last_point.xy(), new_point.z) ) ) ) {
                visible = false;
                return false;
            }
        }
...
int map::obstacle_coverage( const tripoint_bub_ms_ib&loc1, const tripoint_bub_ms_ib&loc2 ) const
{
    // Can't hide if you are standing on furniture, or non-flat slowing-down terrain tile.
    if( furn( loc2 )->id || ( move_cost( loc2 ) > 2 &&
    ...
    tripoint_bub_ms_ib obstaclepos;
    bresenham( loc2.raw(), loc1.raw(), offset, 0, [&obstaclepos](const tripoint& new_point) {
        ...
        obstaclepos = tripoint_bub_ms_ib::make_unchecked(new_point);
        return false;
    } );
...
int map::ledge_coverage( const Creature &viewer, const tripoint_bub_ms_ib&target_p ) const
{
    ...
    if (const std::optional<tripoint_bub_ms_ib> viewer_p = make_inbounds(viewer.pos_bub())) {
        ...
        debugmsg("ERROR: Creature is out of bounds.");
        return 0;
    }
}

int map::ledge_coverage( const tripoint_bub_ms_ib&viewer_p, const tripoint_bub_ms_ib&target_p,
                         const float &eye_level ) const
{
    ...
    const bool higher = viewer_p.z() > target_p.z();
    const tripoint_bub_ms_ib& high_p = higher ? viewer_p : target_p;
    const tripoint_bub_ms_ib& low_p = higher ? target_p : viewer_p;

    tripoint_bub_ms_ib ledge_p = high_p;
    for(tripoint_bub_ms_ib p : line_to(tripoint_bub_ms_ib::make_unchecked( low_p.xy(), high_p.z() ), high_p ) ) {
    ...
    float dist_to_ledge_base = trig_dist( viewer_p, tripoint_bub_ms_ib::make_unchecked( ledge_p.xy(), viewer_p.z() ) );
    ...
    const float flat_dist = trig_dist( viewer_p, tripoint_bub_ms_ib::make_unchecked( target_p.xy(), viewer_p.z() ) );
    ...
    if( target_furn->id || ( move_cost( target_p ) > 2 &&
...
int map::coverage( const tripoint_bub_ms_ib&p ) const
{
...
std::vector<tripoint_bub_ms_ib> map::find_clear_path( const tripoint_bub_ms_ib&source,
        const tripoint_bub_ms_ib&destination ) const
{
...
void map::reachable_flood_steps( std::vector<tripoint_bub_ms_ib> &reachable_pts, const tripoint_bub_ms_ib&f,
                                 int range, const int cost_min, const int cost_max ) const
{
    ...
    for( const tripoint_bub_ms_ib&p : points_in_radius( f, range ) ) {
        const tripoint_bub_ms_ib tp = tripoint_bub_ms_ib::make_unchecked(p.xy(), f.z());
    ...
    for( int y = 0, ndx = 0; y < grid_dim; ++y ) {
        for( int x = 0; x < grid_dim; ++x, ++ndx ) {
            if( o_grid[ ndx ] ) {
                ...
                reachable_pts.push_back( tripoint_bub_ms_ib::make_unchecked(t) );
            }
        }
    }
}

bool map::clear_path( const tripoint_bub_ms_ib&f, const tripoint_bub_ms_ib&t, const int range,
                      const int cost_min, const int cost_max ) const
{
    ...
    if( f.z() == t.z()) {
        ...
        [this, &is_clear, cost_min, cost_max, &t]( const point & new_point ) {
            ...
            // This is a bug. Not taking Z level into account. Fix later?
            const int cost = this->move_cost( point_bub_ms_ib::make_unchecked(new_point) );
    ...
    bool is_clear = true;
    tripoint_bub_ms_ib last_point = f;
    bresenham( f.raw(), t.raw(), 0, 0,
    [this, &is_clear, cost_min, cost_max, t, &last_point]( const tripoint & new_point_ob ) {
        const tripoint_bub_ms_ib new_point = tripoint_bub_ms_ib::make_unchecked(new_point_ob);
        // Exit before checking the last square, it's still reachable even if it is an obstacle.
        if( new_point == t ) {
            return false;
        }
        ...
        if( new_point.z() == last_point.z()) {
            ...
            const int max_z = std::max( new_point.z(), last_point.z());
            const tripoint_bub_ms_ib new_max = tripoint_bub_ms_ib::make_unchecked(new_point.xy(), max_z);
            if( !has_floor_or_support( new_max ) ) {
                const tripoint_bub_ms_ib new_last = tripoint_bub_ms_ib::make_unchecked(new_point.xy(), last_point.z());
                const int cost = move_cost(new_last);
                if( cost > cost_min && cost < cost_max ) {
                    this_clear = true;
                }
            }

            const tripoint_bub_ms_ib last_max = tripoint_bub_ms_ib::make_unchecked(last_point.xy(), max_z);
            if( !this_clear && has_floor_or_support(last_max) ) {
                const tripoint_bub_ms_ib last_new = tripoint_bub_ms_ib::make_unchecked(last_point.xy(), new_point.z());
                const int cost = move_cost( last_new );
                if( cost > cost_min && cost < cost_max ) {
                    this_clear = true;
                }
            }
...
bool map::accessible_items( const tripoint_bub_ms_ib&t ) const
{
...
void map::save()
{
    for( int gridx = 0; gridx < my_MAPSIZE; gridx++ ) {
        for( int gridy = 0; gridy < my_MAPSIZE; gridy++ ) {
            if( zlevels ) {
                for( int gridz = -OVERMAP_DEPTH; gridz <= OVERMAP_HEIGHT; gridz++ ) {
                    saven( tripoint_bub_sm_ib::make_unchecked( gridx, gridy, gridz ) );
                }
            } else {
                saven(tripoint_bub_sm_ib::make_unchecked( gridx, gridy, abs_sub.z() ) );
            }
        }
    }
}
                ...
                const bool pump_events )
{
    ...
    for( int gridx = 0; gridx < my_MAPSIZE; gridx++ ) {
        for( int gridy = 0; gridy < my_MAPSIZE; gridy++ ) {
            loadn(point_bub_sm_ib::make_unchecked( gridx, gridy ), update_vehicle );
            if( pump_events ) {
    ...
    for( int gridx = 0; gridx < my_MAPSIZE; gridx++ ) {
        for( int gridy = 0; gridy < my_MAPSIZE; gridy++ ) {
            ...
            for( int gridz = zmin; gridz <= zmax; gridz++ ) {
                actualize(tripoint_bub_sm_ib::make_unchecked(gridx, gridy, gridz ) );
                if( pump_events ) {
                    inp_mngr.pump_events();
                }
            }
...
void map::shift_traps( const tripoint_rel_sm& shift )
{
    // Offset needs to have sign opposite to shift direction
    const tripoint_rel_ms offset( -shift.x() * SEEX, -shift.y() * SEEY, -shift.z() );
    for( int i = field_furn_locs.size() - 1; i >= 0; --i ) {
        const tripoint_bub_ms pos = field_furn_locs[i] + offset;
        if( const std::optional< tripoint_bub_ms_ib> ib = make_inbounds(pos)) {
            field_furn_locs[i] = *ib;
        } else {
            using std::swap;
            swap(field_furn_locs[i], field_furn_locs.back());
            field_furn_locs.pop_back();
        }
    }
    for (int i = field_ter_locs.size() - 1; i >= 0; --i) {
        const tripoint_bub_ms pos = field_ter_locs[i] + offset;
        if (const std::optional< tripoint_bub_ms_ib> ib = make_inbounds(pos)) {
            field_ter_locs[i] = *ib;
        } else {
            using std::swap;
            swap(field_ter_locs[i], field_ter_locs.back());
            field_ter_locs.pop_back();
        }
    }
    for( auto &traps : traplocs ) {
        for (int i = traps.size() - 1; i >= 0; --i) {
            const tripoint_bub_ms pos = traps[i] + offset;
            if (const std::optional< tripoint_bub_ms_ib> ib = make_inbounds(pos)) {
                traps[i] = *ib;
            } else {
                using std::swap;
                swap(traps[i], traps.back());
                traps.pop_back();
            }
        }
    }
...
void map::shift( const point_rel_sm &sp )
{
    ...
    shift_traps(tripoint_rel_sm( sp, 0 ) );
    ...
    for( int gridz = zmin; gridz <= zmax; gridz++ ) {
        ...
        if( sp.x() >= 0) {
            for( int gridx = 0; gridx < my_MAPSIZE; gridx++ ) {
                if( sp.y() >= 0) {
                    for( int gridy = 0; gridy < my_MAPSIZE; gridy++ ) {
                        const tripoint_bub_sm_ib grid = tripoint_bub_sm_ib::make_unchecked( gridx, gridy, gridz );
                        if( gridx + sp.x() < my_MAPSIZE && gridy + sp.y() < my_MAPSIZE ) {
                            copy_grid( grid, tripoint_bub_sm_ib::make_unchecked(grid + sp) );
    ...
    if( !support_cache_dirty.empty() ) {
        std::unordered_set<tripoint_bub_ms_ib> old_cache = std::exchange( support_cache_dirty, std::unordered_set<tripoint_bub_ms_ib>());
        for( const tripoint_bub_ms_ib&pt : old_cache) {
            support_cache_dirty.insert(tripoint_bub_ms_ib::make_unchecked(pt - project_to<coords::scale::map_square>(sp)));
        }
    }
    // actualize after loading all submaps to prevent errors
    // with entities at the edges
    for(const tripoint_bub_sm_ib& loaded_grid : loaded_grids ) {
...
void map::loadn( const tripoint_bub_sm_ib&grid, const bool update_vehicles )
{
    dbg( D_INFO ) << "map::loadn(game[" << g.get() << "], worldx[" << abs_sub.x()
                  << "], worldy[" << abs_sub.y() << "], grid " << grid << ")";

    const tripoint_abs_sm grid_abs_sub = abs_from_bub(grid);
...
void map::loadn( const point_bub_sm_ib&grid, bool update_vehicles )
{
    if( zlevels ) {
        for( int gridz = -OVERMAP_DEPTH; gridz <= OVERMAP_HEIGHT; gridz++ ) {
            loadn(tripoint_bub_sm_ib::make_unchecked( grid, gridz ), update_vehicles );
        }
        ...
        for( int gridz = -OVERMAP_DEPTH; gridz <= OVERMAP_HEIGHT; gridz++ ) {
            add_roofs(tripoint_bub_sm_ib::make_unchecked( grid, gridz ) );
        }
    } else {
        loadn(tripoint_bub_sm_ib::make_unchecked( grid, abs_sub.z() ), update_vehicles );
    }
}

void map::rotten_item_spawn( const item &item, const tripoint_bub_ms_ib &pnt )
{
...
void map::fill_funnels( const tripoint_bub_ms_ib&p, const time_point &since )
{
    ...
    if( biggest_container != items.end() ) {
        retroactively_fill_from_funnel( *biggest_container, tr, since, calendar::turn, abs_from_bub( p ) );
    }
}

void map::grow_plant( const tripoint_bub_ms_ib&p )
{
    const furn_t &furn = this->furn( p ).obj();
    ...
    if( seed == items.end() ) {
        ...
        // TODO: Fix point types
        const tripoint_abs_ms ms_pos( abs_from_bub( p ) );
...
void map::restock_fruits( const tripoint_bub_ms_ib&p, const time_duration &time_since_last_actualize )
{
    const ter_t &ter = this->ter( p ).obj();
...
void map::produce_sap( const tripoint_bub_ms_ib&p, const time_duration &time_since_last_actualize )
{
...
void map::rad_scorch( const tripoint_bub_ms_ib&p, const time_duration &time_since_last_actualize )
{
...
void map::decay_cosmetic_fields( const tripoint_bub_ms_ib&p,
                                 const time_duration &time_since_last_actualize )
{
    for( auto &pr : field_at( p ) ) {
        field_entry &fd = pr.second;
        const time_duration hl = fd.get_field_type()->half_life;
...
void map::actualize( const tripoint_bub_sm_ib&grid )
{
    ...
    for( int x = 0; x < SEEX; x++ ) {
        for( int y = 0; y < SEEY; y++ ) {
            const point_sm_ms_ib p = point_sm_ms_ib::make_unchecked(x, y);
            const tripoint_bub_ms_ib pnt = project_combine( grid, p );
...
void map::add_roofs( const tripoint_bub_sm_ib&grid )
{
    if( !zlevels ) {
        // No roofs required!
    ...
    submap *const sub_below = check_roof ? get_submap_at_grid(tripoint_bub_sm_ib::make_unchecked(grid + tripoint_rel_sm(0, 0, -1) )) : nullptr;
    ...
    for( int x = 0; x < SEEX; x++ ) {
        for( int y = 0; y < SEEY; y++ ) {
            const point_sm_ms_ib p = point_sm_ms_ib::make_unchecked(x, y);
            const ter_id ter_here = sub_here->get_ter( p );
            ...
            if( !check_roof ) {
                ...
                sub_here->set_ter( p, t_rock_floor );
                continue;
            }

            const ter_t &ter_below = sub_below->get_ter( p ).obj();
            if( ter_below.roof ) {
                // TODO: Make roof variable a ter_id to speed this up
                sub_here->set_ter( p, ter_below.roof.id() );
            }
        }
    }
}
...
void map::spawn_monsters_submap_group( const tripoint_bub_sm_ib&gp, mongroup &group, bool ignore_sight )
{
    Character &player_character = get_player_character();
    ...
    std::vector<tripoint_bub_ms_ib> locations;
    if( !ignore_sight ) {
    ...
    static const auto allow_on_terrain = [&]( const tripoint_bub_ms_ib & p ) {
    ...
    if( current_submap->is_uniform() ) {
        const tripoint_bub_ms_ib upper_left = project_to<coords::scale::map_square>(gp);
    ...
    for( int x = 0; x < SEEX; ++x ) {
        for( int y = 0; y < SEEY; ++y ) {
            const point_sm_ms_ib p = point_sm_ms_ib::make_unchecked(x, y);
            const tripoint_bub_ms_ib fp = project_combine(gp, p);
            ...
            if (!ignore_sight) {
                if (const std::optional<tripoint_bub_ms_ib> ib = make_inbounds(player_character.pos_bub());
                    ib && sees(*ib, fp, s_range) ) {
    ...
    for( monster &tmp : group.monsters ) {
        for( int tries = 0; tries < 10 && !locations.empty(); tries++ ) {
            const tripoint_bub_ms_ib local_pos = random_entry_removed( locations );
            const tripoint_abs_ms abs_pos = get_map().abs_from_bub( local_pos );
            if( !tmp.can_move_to( local_pos.raw() ) ) {
            ...
            monster *const placed = g->place_critter_at( make_shared_fast<monster>( tmp ), local_pos.raw() );
            if( placed ) {
                placed->on_load();
            }
            break;
...
void map::spawn_monsters_submap( const tripoint_bub_sm_ib&gp, bool ignore_sight, bool spawn_nonlocal )
{
    const tripoint_abs_sm submap_pos = abs_from_bub( gp );
    ...
    for( size_t sp_i = 0; sp_i < current_submap->spawns.size(); ++sp_i ) {
        ...
        const tripoint_bub_ms_ib center = project_combine(gp, i.pos);
        const tripoint_range<tripoint_bub_ms_ib> points = points_in_radius( center, 3 );
        ...
        for( int j = 0; j < i.count; j++ ) {
            ...
            const auto valid_location = [&]( const tripoint_bub_ms_ib& p ) {
                ...
                return ( !g || &get_map() != this || !creatures.creature_at( p ) ) && tmp.can_move_to( p.raw() );
            };
            ...
            if( valid_location( center ) ) {
                place_it( center.raw() );
            } else if( const std::optional<tripoint_bub_ms_ib> pos = random_point( points, valid_location ) ) {
                place_it( pos->raw() );
            }
        }
    }
    ...
    for( int gz = zmin; gz <= zmax; gz++ ) {
        for( int gx = 0; gx < my_MAPSIZE; gx++ ) {
            for( int gy = 0; gy < my_MAPSIZE; gy++ ) {
                spawn_monsters_submap( tripoint_bub_sm_ib::make_unchecked(gx, gy, gz), ignore_sight, spawn_nonlocal );
            }
        }
    }
}
...
const std::vector<tripoint_bub_ms_ib> &map::get_furn_field_locations() const
{
    return field_furn_locs;
}

const std::vector<tripoint_bub_ms_ib> &map::get_ter_field_locations() const
{
    return field_ter_locs;
}

const std::vector<tripoint_bub_ms_ib> &map::trap_locations( const trap_id &type ) const
{
...
{
    static constexpr tripoint_bub_ms map_boundary_min( 0, 0, -OVERMAP_DEPTH );
    static constexpr tripoint_bub_ms map_boundary_max( MAPSIZE_X, MAPSIZE_Y, OVERMAP_HEIGHT + 1 );

    static constexpr half_open_cuboid<tripoint_bub_ms> map_boundaries(
...
bool tinymap::inbounds( const tripoint_bub_ms&p ) const
{
    constexpr tripoint_bub_ms map_boundary_min( 0, 0, -OVERMAP_DEPTH );
    constexpr tripoint_bub_ms map_boundary_max( SEEX * 2, SEEY * 2, OVERMAP_HEIGHT + 1 );

    constexpr half_open_cuboid<tripoint_bub_ms> map_boundaries( map_boundary_min, map_boundary_max );
...
fake_map::fake_map( const ter_id &ter_type )
{
    ...
    for( int gridx = 0; gridx < my_MAPSIZE; gridx++ ) {
        for( int gridy = 0; gridy < my_MAPSIZE; gridy++ ) {
            ...
            setsubmap( get_nonant( tripoint_bub_sm_ib::make_unchecked( gridx, gridy, fake_map_z ) ), sm.get() );
...
void map::set_graffiti( const tripoint_bub_ms_ib&p, const std::string &contents )
{
    point_sm_ms_ib l;
    submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to set graffiti at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return;
    }
    current_submap->set_graffiti( l, contents);
}

void map::delete_graffiti( const tripoint_bub_ms_ib&p )
{
    point_sm_ms_ib l;
    submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to delete graffiti at (%d,%d) but the submap is not loaded", p.x(), p.y());
        return;
    }
    current_submap->delete_graffiti(l);
}

const std::string &map::graffiti_at( const tripoint_bub_ms_ib&p ) const
{
    point_sm_ms_ib l;
    const submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to get graffiti at (%d,%d) but the submap is not loaded", p.x(), p.y());
...
bool map::has_graffiti_at( const tripoint_bub_ms_ib&p ) const
{
    point_sm_ms_ib l;
    const submap *const current_submap = get_submap_at( p, l );
    if( current_submap == nullptr ) {
        debugmsg( "Tried to get graffiti at (%d,%d) but the submap is not loaded", p.x(), l.y());
        return false;
    }
    return current_submap->has_graffiti(l);
}

int map::determine_wall_corner( const tripoint_bub_ms_ib&p ) const
{
    const std::bitset<NUM_TERCONN> &test_connect_group = ter( p )->connect_to_groups;
    ...
    switch( connections ) {
        ...
        case 0 | 0 | 0 | 0:
            return ter( p )->symbol(); // technically just a column
...
void map::build_outside_cache( const int zlev )
{
    ...
    for( int smx = 0; smx < my_MAPSIZE; ++smx ) {
        for( int smy = 0; smy < my_MAPSIZE; ++smy ) {
            const submap *cur_submap = get_submap_at_grid(tripoint_bub_sm_ib::make_unchecked( smx, smy, zlev ) );
            if( cur_submap == nullptr ) {
            ...
            const point_bub_sm_ib g = point_bub_sm_ib::make_unchecked(smx, smy);
            for( int sx = 0; sx < SEEX; ++sx ) {
                for( int sy = 0; sy < SEEY; ++sy ) {
                    const point_sm_ms_ib sp = point_sm_ms_ib::make_unchecked( sx, sy );
                    if( cur_submap->get_ter( sp )->has_flag( ter_furn_flag::TFLAG_INDOORS ) ||
                        cur_submap->get_furn( sp )->has_flag( ter_furn_flag::TFLAG_INDOORS ) ) {
                        const point_bub_ms p = project_combine(g, sp);
                        // Add 1 to both coordinates, because we're operating on the padded cache
                        for( int dx = 0; dx <= 2; dx++ ) {
                            for( int dy = 0; dy <= 2; dy++ ) {
                                padded_cache[p + point_rel_ms(dx, dy)] = false;
                            }
                        }
                    }
                }
...
void map::build_obstacle_cache(
    const tripoint_bub_ms_ib&start, const tripoint_bub_ms_ib&end,
    cata::mdarray<fragment_cloud, point_bub_ms_ib> &obstacle_cache )
{
    ...
    for( int smx = min_submap.x; smx <= max_submap.x; ++smx ) {
        for( int smy = min_submap.y; smy <= max_submap.y; ++smy ) {
            const submap *cur_submap = get_submap_at_grid(tripoint_bub_sm_ib::make_unchecked(smx, smy, start.z()));
            if( cur_submap == nullptr ) {
            ...
            for( int sx = 0; sx < SEEX; ++sx ) {
                for( int sy = 0; sy < SEEY; ++sy ) {
                    const point_sm_ms_ib sp = point_sm_ms_ib::make_unchecked( sx, sy );
                    int ter_move = cur_submap->get_ter( sp )->movecost;
                    int furn_move = cur_submap->get_furn( sp )->movecost;
                    const point_bub_ms_ib p2 = project_combine(g, sp);
                    if( ter_move == 0 || furn_move < 0 || ter_move + furn_move == 0 ) {
                        obstacle_cache[p2].velocity = 1000.0f;
                        obstacle_cache[p2].density = 0.0f;
                    } else {
                        ...
                        obstacle_cache[p2].velocity = 1.2f;
                        obstacle_cache[p2].density = 1.0f;
                    }
                }
            }
        }
    ...
    for( wrapped_vehicle &v : vehs ) {
        for( const vpart_reference &vp : v.v->get_all_parts() ) {
            if (const std::optional<tripoint_bub_ms_ib> p = make_inbounds(v.pos + vp.part().precalc[0])) {
                if (p->z() != start.z()) {
                    break;
                }
                if (vp.obstacle_at_part()) {
                    const point_bub_ms_ib& ib = p->xy();
                    obstacle_cache[ib].velocity = 1000.0f;
                    obstacle_cache[ib].density = 0.0f;
                }
            }
        }
    }
    ...
    for( Creature &critter : g->all_creatures() ) {
        ...
        if (const std::optional<tripoint_bub_ms_ib> p = make_inbounds(loc)) {
            if (p->z() != start.z()) {
                break;
            }
            ...
            obstacle_cache[ib].density = 1.0 - critter.ranged_target_size();
        }
    }
}
...
bool map::build_floor_cache( const int zlev )
{
    ...
    for( int smx = 0; smx < my_MAPSIZE; ++smx ) {
        for( int smy = 0; smy < my_MAPSIZE; ++smy ) {
            const submap *cur_submap = get_submap_at_grid(tripoint_bub_sm_ib::make_unchecked(smx, smy, zlev) );
            const submap *below_submap = !lowest_z_lev ? get_submap_at_grid(tripoint_bub_sm_ib::make_unchecked(smx, smy, zlev - 1) ) : nullptr;
            ...
            for( int sx = 0; sx < SEEX; ++sx ) {
                for( int sy = 0; sy < SEEY; ++sy ) {
                    ...
                    if( terrain.has_flag( ter_furn_flag::TFLAG_NO_FLOOR ) ||
                        ...
                        if( below_submap &&
                            below_submap->get_furn( sp )->has_flag( ter_furn_flag::TFLAG_SUN_ROOF_ABOVE ) ) {
                            continue;
                        }
                        const point_bub_ms_ib p = project_combine(g, sp);
                        floor_cache[p] = false;
                        no_floor_gaps = false;
                    }
                }
...
static void vehicle_caching_internal( const tripoint_bub_ms_ib& pos, level_cache &zch, const vpart_reference &vp, vehicle *v )
{
    ...
    if( vehicle_is_opaque ) {
        ...
        if( dpart < 0 || !v->part( dpart ).open ) {
            transparency_cache[pos.xy()] = LIGHT_TRANSPARENCY_SOLID;
        } else {
    ...
    if( vehicle_is_opaque || vp.is_inside() ) {
        outside_cache[pos.xy()] = false;
    }

    if( vp.has_feature( VPFLAG_BOARDABLE ) && !vp.part().is_broken() ) {
        floor_cache[pos.xy()] = true;
    }
}

static void vehicle_caching_internal_above( const tripoint_bub_ms_ib& pos, level_cache &zch_above, const vpart_reference &vp,
        vehicle *v )
{
    if( vp.has_feature( VPFLAG_ROOF ) || vp.has_feature( VPFLAG_OPAQUE ) ) {
        zch_above.floor_cache[pos.xy()] = true;
    }
}
...
void map::do_vehicle_caching( int z )
{
    ...
    for( vehicle *v : ch->vehicle_list ) {
        for( const vpart_reference &vp : v->get_all_parts_with_fakes() ) {
            if (const std::optional<tripoint_bub_ms_ib> part_pos = make_inbounds(v->bub_part_pos(vp.part()))) {
                vehicle_caching_internal(*part_pos, get_cache(part_pos->z()), vp, v);
                if (part_pos->z() < OVERMAP_HEIGHT) {
                    vehicle_caching_internal_above(*part_pos, get_cache(part_pos->z() + 1), vp, v);
                }
            }
        }
    }
...
void map::build_map_cache( const int zlev, bool skip_lightmap )
{
    ...
    if( seen_cache_dirty ) {
        if( const std::optional<tripoint_bub_ms_ib> ib = make_inbounds( bub_from_abs( p ) ) ) {
            build_seen_cache( *ib, zlev, sr );
        }
        player_prev_pos = p;
    ...
    if( camera_cache_dirty ) {
        ...
        for( Character::cached_moncam const &mon : u.moncam_cache ) {
            if (const std::optional<tripoint_bub_ms_ib> ib = make_inbounds(bub_from_abs(mon.second))) {
                int const range = mon.first->type->vision_day;
                build_seen_cache( *ib, ib->z(), range, cumulative,
...
tripoint_abs_ms map::abs_from_bub(const tripoint_bub_ms& p) const
{
                         ...
                         bool avoid_creatures )
{
    draw_line( [this, type, avoid_creatures]( const point& p ) {
        if (const std::optional<point_bub_ms_ib> ib = make_inbounds(point_bub_ms(p))) {
            this->ter_set(*ib, type, avoid_creatures);
        }
    }, p1.raw(), p2.raw());
}
                          ...
                          bool avoid_creatures )
{
    draw_line( [this, type, avoid_creatures]( const point& p ) {
        if (const std::optional<point_bub_ms_ib> ib = make_inbounds(point_bub_ms(p))) {
            this->furn_set(*ib, type, avoid_creatures);
        }
    }, p1.raw(), p2.raw());
}
...
void map::draw_fill_background( const ter_id &type )
{
    ...
    for( int gridx = 0; gridx < my_MAPSIZE; gridx++ ) {
        for( int gridy = 0; gridy < my_MAPSIZE; gridy++ ) {
            submap *sm = get_submap_at_grid(point_bub_sm_ib::make_unchecked(gridx, gridy));
            if( sm == nullptr ) {
...
{
    draw_square_ter( f, point_bub_ms_ib(), point_bub_ms_ib::make_unchecked(SEEX * my_MAPSIZE - 1, SEEY * my_MAPSIZE - 1));
}
...
void map::draw_square_ter( ter_id( *f )(), const point_bub_ms&p1, const point_bub_ms&p2, bool avoid_creatures )
{
    draw_square( [this, f, avoid_creatures]( const point& p ) {
        if (const std::optional<point_bub_ms_ib> ib = make_inbounds(point_bub_ms(p))) {
            this->ter_set(*ib, f(), avoid_creatures);
        }
    }, p1.raw(), p2.raw());
}
                           ...
                           const point_bub_ms&p2, bool avoid_creatures )
{
    draw_square( [this, f, avoid_creatures]( const point& p ) {
        if (const std::optional<point_bub_ms_ib> ib = make_inbounds(point_bub_ms(p))) {
            ...
            this->ter_set(*ib, tid != nullptr ? *tid : t_null, avoid_creatures);
        }
    }, p1.raw(), p2.raw());
}
...
{
    draw_rough_circle( [this, type]( const point& p ) {
        if (const std::optional<tripoint_bub_ms_ib> ib = make_inbounds(point_bub_ms(p), abs_sub.z())) {
            if (!is_open_air(*ib)) {
                this->furn_set(*ib, type);
            }
        }
    }, p.raw(), rad );
}
...
void map::add_corpse( const tripoint_bub_ms_ib &p )
{
    item body;
...
field &map::get_field( const tripoint_bub_ms_ib&p )
{
    return field_at( p );
}
...
void map::creature_on_trap( Creature &c, const bool may_avoid ) const
{
    ...
    maybe_trigger_trap( *make_inbounds(c.pos_bub()), c, may_avoid );
}

void map::maybe_trigger_trap( const tripoint_bub_ms_ib&pos, Creature &c, const bool may_avoid ) const
{
    ...
    if (!tr.has_flag(json_flag_UNDODGEABLE) && may_avoid && c.avoid_trap(pos.raw(), tr)) {
        ...
        if (!tr.is_always_invisible() && pl && !pl->knows_trap(pos.raw())) {
...
void function_over( const map& here, const tripoint_bub_ms &start, const tripoint_bub_ms& end, Functor fun )
{
    // start and end are just two points, end can be "before" start
    // Also clip the area to map area
    const tripoint_bub_ms min = coords::coord_max(coords::coord_min(start, end), tripoint_bub_ms(0, 0, -OVERMAP_DEPTH));
    const tripoint_bub_ms max = coords::coord_min(coords::coord_max(start, end), tripoint_bub_ms(SEEX * here.my_MAPSIZE - 1, SEEY * here.my_MAPSIZE - 1, OVERMAP_HEIGHT));

    // Submaps that contain the bounding points
    const point_bub_sm min_sm = project_to<coords::scale::submap>(min.xy());
    const point_bub_sm max_sm = project_to<coords::scale::submap>(max.xy());

    // Z outermost, because submaps are flat
    for( int z = min.z(); z <= max.z(); z++ ) {
        for( int smx = min_sm.x(); smx <= max_sm.x(); smx++ ) {
            for( int smy = min_sm.y(); smy <= max_sm.y(); smy++ ) {
                const tripoint_bub_sm_ib g = tripoint_bub_sm_ib::make_unchecked(smx, smy, z);
                submap const *cur_submap = here.get_submap_at_grid( g );
                if( cur_submap == nullptr ) {
                    debugmsg( "Tried to function over (%d,%d,%d) but the submap is not loaded", smx, smy, z );
                    continue;
                }
                // Bounds on the submap coordinates
                const point sm_min( smx > min_sm.x ? 0 : min.x % SEEX,
                ...
                for( int sx = sm_min.x; sx <= sm_max.x; ++sx ) {
                    for( int sy = sm_min.y; sy <= sm_max.y; ++sy ) {
                        const point_sm_ms_ib lp = point_sm_ms_ib::make_unchecked(sx, sy);
                        const iteration_state rval = fun( g, cur_submap, lp );
...
void map::scent_blockers( cata::mdarray<bool, point_bub_ms_ib> &blocks_scent,
                          cata::mdarray<bool, point_bub_ms_ib> &reduces_scent,
                          const point_bub_ms_ib&min, const point_bub_ms_ib&max )
{
    ter_furn_flag reduce = ter_furn_flag::TFLAG_REDUCE_SCENT;
    ter_furn_flag block = ter_furn_flag::TFLAG_NO_SCENT;
    auto fill_values = [&]( const tripoint_bub_sm_ib& gp, const submap * sm, const point_sm_ms_ib& lp ) {
        // We need to generate the x/y coordinates, because we can't get them "for free"
        const point_bub_ms_ib p = project_combine(gp, lp).xy();
        if( sm->get_ter( lp )->has_flag( block ) ) {
            blocks_scent[p] = true;
            reduces_scent[p] = false;
        } else if( sm->get_ter( lp )->has_flag( reduce ) ||
                   sm->get_furn( lp )->has_flag( reduce ) ) {
            blocks_scent[p] = false;
            reduces_scent[p] = true;
        } else {
            blocks_scent[p] = false;
            reduces_scent[p] = false;
        }
    ...
    function_over( *this, tripoint_bub_ms( min, abs_sub.z() ), tripoint_bub_ms( max, abs_sub.z() ), fill_values );
...
tripoint_range<tripoint_bub_ms_ib> map::points_in_rectangle( const tripoint_bub_ms&from, const tripoint_bub_ms&to ) const
{
    const tripoint_bub_ms lower(0, 0, -OVERMAP_DEPTH);
    const tripoint_bub_ms upper(SEEX * my_MAPSIZE - 1, SEEY * my_MAPSIZE - 1, OVERMAP_HEIGHT);
    const tripoint_bub_ms_ib min = tripoint_bub_ms_ib::make_unchecked(coords::coord_max(coords::coord_min(from, to), lower));
    const tripoint_bub_ms_ib max = tripoint_bub_ms_ib::make_unchecked(coords::coord_min(coords::coord_max(from, to), upper));
    return tripoint_range<tripoint_bub_ms_ib>( min, max );
}

tripoint_range<tripoint_bub_ms_ib> map::points_in_radius( const tripoint_bub_ms &center, size_t radius,
        size_t radiusz ) const
{
    const tripoint_rel_ms offset(radius, radius, radiusz);
    return points_in_rectangle(center - offset, center + offset);
}

tripoint_range<tripoint_bub_ms_ib> map::points_on_zlevel( const int z ) const
{
    if( z < -OVERMAP_DEPTH || z > OVERMAP_HEIGHT ) {
        return tripoint_range<tripoint_bub_ms_ib>( );
    }
    const tripoint_bub_ms_ib lower = tripoint_bub_ms_ib::make_unchecked(0, 0, z);
    const tripoint_bub_ms_ib upper = tripoint_bub_ms_ib::make_unchecked(SEEX* my_MAPSIZE - 1, SEEY* my_MAPSIZE - 1, z);
    return tripoint_range<tripoint_bub_ms_ib>(lower, upper);
}

tripoint_range<tripoint_bub_ms_ib> map::points_on_zlevel() const
{
    return points_on_zlevel(abs_sub.z());
}

std::vector<tripoint_bub_ms_ib> map::find_furnitures_with_flag_in_radius( const tripoint_bub_ms&center, size_t radius, const std::string &flag, size_t radiusz ) const
{
    std::vector<tripoint_bub_ms_ib> furn_locs;
    for( const tripoint_bub_ms_ib &furn_loc : points_in_radius( center, radius, radiusz ) ) {
...
std::vector<tripoint_bub_ms_ib> map::find_furnitures_with_flag_in_radius( const tripoint_bub_ms&center, size_t radius, const ter_furn_flag flag, size_t radiusz ) const
{
    std::vector<tripoint_bub_ms_ib> furn_locs;
    for( const tripoint_bub_ms_ib&furn_loc : points_in_radius( center, radius, radiusz ) ) {
        if( has_flag_furn( flag, furn_loc ) ) {
...
std::vector<Creature *> map::get_creatures_in_radius( const tripoint_bub_ms&center, size_t radius, size_t radiusz ) const
{
    creature_tracker &creatures = get_creature_tracker();
    std::vector<Creature *> creature_list;
    for( const tripoint_bub_ms_ib&loc : points_in_radius( center, radius, radiusz ) ) {
...
void map::update_pathfinding_cache( int zlev ) const
{
    ...
    for( int smx = 0; smx < my_MAPSIZE; ++smx ) {
        for( int smy = 0; smy < my_MAPSIZE; ++smy ) {
            const tripoint_bub_sm_ib g = tripoint_bub_sm_ib::make_unchecked(smx, smy, zlev);
            const submap *cur_submap = get_submap_at_grid(g);
            if( !cur_submap ) {
                return;
            }
            ...
            for( int sx = 0; sx < SEEX; ++sx ) {
                for( int sy = 0; sy < SEEY; ++sy ) {
                    const point_sm_ms_ib s = point_sm_ms_ib::make_unchecked(sx, sy);
                    const tripoint_bub_ms_ib p = project_combine(g, s);

                    pf_special cur_value = PF_NORMAL;

                    const_maptile tile( cur_submap, s );
                    ...
                    if( ( !tile.get_trap_t().is_benign() || !terrain.trap->is_benign() ) &&
...
bool map::is_cornerfloor( const tripoint_bub_ms_ib &p ) const
{
    if( impassable( p ) ) {
        return false;
    }
    std::unordered_set<tripoint_bub_ms> impassable_adjacent;
    for( const tripoint_bub_ms_ib &pt : points_in_radius( p, 1 ) ) {
        if( impassable( pt ) ) {
    ...
    if( !impassable_adjacent.empty() ) {
        ...
        std::unordered_set<tripoint_bub_ms> diagonals = { p + tripoint_north_east, p + tripoint_north_west, p + tripoint_south_east, p + tripoint_south_west };
        for( const tripoint_bub_ms&impassable_diagonal : diagonals ) {
            if( impassable_adjacent.count( impassable_diagonal ) != 0 ) {
                ...
                int f = 0;
                for( const tripoint_bub_ms_ib &l : points_in_radius( impassable_diagonal, 1 ) ) {
    ...
    for( int sz = 1; sz <= OVERMAP_HEIGHT; sz++ ) {
        ...
        for( int sx = 0; sx < my_MAPSIZE; sx++ ) {
            for( int sy = 0; sy < my_MAPSIZE; sy++ ) {
                const tripoint_bub_sm_ib s = tripoint_bub_sm_ib::make_unchecked(sx, sy, sz);
                const submap *sm = get_submap_at_grid( s );
                if( sm == nullptr ) {
...
bool map::has_potential_los( const tripoint_bub_ms_ib &from, const tripoint_bub_ms_ib&to ) const
{
    if( from.z() == to.z()) {
    ...
    tripoint_bub_ms_ib upper;
    tripoint_bub_ms_ib lower;
...
int map::reachability_cache_value( const tripoint_bub_ms &p, bool vertical_cache,
                                   reachability_cache_quadrant quadrant ) const
{
    if (const std::optional<tripoint_bub_ms_ib> ib = make_inbounds(p)) {
        // rebuild caches, so valid values are shown
        has_potential_los(*ib, *ib); // rebuild horizontal cache;

        if (const std::optional<tripoint_bub_ms_ib> uib = make_inbounds(p + tripoint_above)) {
            has_potential_los(*ib, *uib); // rebuild "up" cache
        }

        const level_cache& lc = get_cache(p.z());
        if (vertical_cache) {
            return lc.r_up_cache->get_value(quadrant, ib->xy());
        } else {
            return lc.r_hor_cache->get_value(quadrant, ib->xy());
        }
    }
    return -2;
}
...
bool map::has_haulable_items( const tripoint_bub_ms_ib &pos )
{
...
std::vector<item_location> map::get_haulable_items( const tripoint_bub_ms_ib&pos )
{
    ...
    for( item &it : items ) {
        if( is_haulable( it ) ) {
            target_items.emplace_back( map_cursor( abs_from_bub( pos ) ), &it );
        }
    }
    return target_items;
}
